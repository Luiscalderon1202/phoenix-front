...
import { MotivoAnulacionApi } from '@phoenix/inventarios/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  MOTIVO_ANULACION_MAX_ABREVIATURA,
  MOTIVO_ANULACION_MAX_CODIGO_CONTABLE,
  MOTIVO_ANULACION_MAX_NOMBRE,
  type MotivoAnulacion,
  type MotivoAnulacionInput,
...
/**
 * Mantenimiento de Motivos de anulación.
 *
 ...
 */
@Component({
  selector: 'erp-motivo-anulacion-list-page',
  ...
  templateUrl: './motivo-anulacion-list-page.html',
  styleUrl: './motivo-anulacion-list-page.scss',
})
export class MotivoAnulacionListPage {
  private readonly api = inject(MotivoAnulacionApi);
  ...
  protected readonly maxAbreviatura = MOTIVO_ANULACION_MAX_ABREVIATURA;
  protected readonly maxCodigoContable = MOTIVO_ANULACION_MAX_CODIGO_CONTABLE;
  protected readonly maxNombre = MOTIVO_ANULACION_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<MotivoAnulacion[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de motivos de anulación.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<MotivoAnulacion[]>(() => {
  ...
  protected readonly rowsPagina = computed<MotivoAnulacion[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<MotivoAnulacion>[] = [
   ...
   */
  protected async onToggleEstado(row: MotivoAnulacion): Promise<void> {
  ...
  private patchRow(id: number, patch: Partial<MotivoAnulacion>): void {
    this.items.update((list) => list.map((it) => (it.motivoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<MotivoAnulacion[]>([]);

  protected onSeleccion(filas: MotivoAnulacion[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar motivos de anulación',
   ...
   */
  protected async onReorder(filas: MotivoAnulacion[]): Promise<void> {
  ...
  protected readonly editando = signal<MotivoAnulacion | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(MOTIVO_ANULACION_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(MOTIVO_ANULACION_MAX_ABREVIATURA)]],
    codigo_contable: ['', [Validators.maxLength(MOTIVO_ANULACION_MAX_CODIGO_CONTABLE)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar motivo de anulación' : 'Nuevo motivo de anulación',
  );
  ...
  protected onEditar(row: MotivoAnulacion): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: MotivoAnulacionInput = this.form.getRawValue();
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: MotivoAnulacion): Promise<void> {
    const ok = await this.confirm.ask({
      title: 'Eliminar motivo de anulación',
    ...
    try {
      ...
      // 409 si hay ventas anuladas con ese motivo: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    const cabeceras = ['Orden', 'ID', 'Nombre', 'Abreviatura', 'Código', 'Estado'];
    const matriz = [
      cabeceras,
      ...this.rows().map((t) => [
        t.orden,
    ...
    if (e.formato === 'csv') {
      exportCsv('motivos-anulacion.csv', matriz);
    } else {
      exportXls('motivos-anulacion.xls', matriz);
    }
  }
}
