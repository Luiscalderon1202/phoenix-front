...
import type { MotivoAnulacion, MotivoAnulacionInput } from '@phoenix/inventarios/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { MotivoAnulacionListPage } from './motivo-anulacion-list-page';

function fila(over: Partial<MotivoAnulacion> = {}): MotivoAnulacion {
  return {
    motivoid: 1,
    nombre: 'ERROR EN EL RUC',
    abreviatura: 'ERR',
    codigo_contable: '01',
    orden: 1,
    estado: true,
...
class MotivoAnulacionApiMock {
  rows: MotivoAnulacion[] = [fila(), fila({ motivoid: 2, nombre: 'DUPLICADO', abreviatura: 'DUP', codigo_contable: '02', orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: MotivoAnulacionInput[] = [];
  readonly actualizados: { id: number; input: MotivoAnulacionInput }[] = [];
  ...
  create(input: MotivoAnulacionInput) {
  ...
  update(id: number, input: MotivoAnulacionInput) {
...
async function setup(api = new MotivoAnulacionApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [MotivoAnulacionListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: MotivoAnulacionApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(MotivoAnulacionListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): MotivoAnulacionInput };
    rows(): MotivoAnulacion[];
    filtrando(): boolean;
...
describe('MotivoAnulacionListPage', () => {
  ...
  it('filtra por estado', async () => {
    const api = new MotivoAnulacionApiMock();
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'ERROR EN EL MONTO', abreviatura: 'MON', codigo_contable: '03' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'ERROR EN EL MONTO', abreviatura: 'MON', codigo_contable: '03' }]);
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'ERROR EN EL RUC' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('ERROR EN EL RUC');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar motivo de anulación');
    expect(c.form.getRawValue()).toEqual({ nombre: 'ERROR EN EL RUC', abreviatura: 'ERR', codigo_contable: '01' });

    c.form.patchValue({ nombre: 'ERROR EN EL RUC EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('ERROR EN EL RUC EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new MotivoAnulacionApiMock();
    api.rows = [fila(), fila({ motivoid: 2, nombre: 'DUPLICADO', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'DUPLICADO EDITADO' });
    c.onGuardar();
    await asentar(fixture);
