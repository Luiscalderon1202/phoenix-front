...
import type { TipoConsumo, TipoConsumoInput } from '@phoenix/inventarios/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { TipoConsumoListPage } from './tipo-consumo-list-page';

function fila(over: Partial<TipoConsumo> = {}): TipoConsumo {
  return {
    tipoid: 1,
    nombre: 'SALÓN',
    abreviatura: 'SAL',
...
class TipoConsumoApiMock {
  rows: TipoConsumo[] = [fila(), fila({ tipoid: 2, nombre: 'PARA LLEVAR', abreviatura: 'LLE', orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: TipoConsumoInput[] = [];
  readonly actualizados: { id: number; input: TipoConsumoInput }[] = [];
  ...
  update(id: number, input: TipoConsumoInput) {
...
async function setup(api = new TipoConsumoApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [TipoConsumoListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: TipoConsumoApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(TipoConsumoListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): TipoConsumoInput };
    rows(): TipoConsumo[];
    filtrando(): boolean;
...
describe('TipoConsumoListPage', () => {
  ...
  it('pinta el catálogo que devuelve el backend', async () => {
    ...
    expect(celdas(el, 1)).toEqual(['SALÓN', 'PARA LLEVAR']);
  });
  ...
  it('filtra por estado', async () => {
    const api = new TipoConsumoApiMock();
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'DELIVERY', abreviatura: 'DEL' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'DELIVERY', abreviatura: 'DEL' }]);
  ...
  it('el alta añade la fila al final, que es donde la coloca el backend', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'DELIVERY' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.rows().map((row) => row.nombre).at(-1)).toBe('DELIVERY');
  });
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'SALÓN' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('SALÓN');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar tipo de consumo');
    expect(c.form.getRawValue()).toEqual({ nombre: 'SALÓN', abreviatura: 'SAL' });

    c.form.patchValue({ nombre: 'SALÓN EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('SALÓN EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new TipoConsumoApiMock();
    api.rows = [fila(), fila({ tipoid: 2, nombre: 'PARA LLEVAR', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'PARA LLEVAR EDITADO' });
    c.onGuardar();
    await asentar(fixture);
