...
import { TipoConsumoApi } from '@phoenix/inventarios/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  TIPO_CONSUMO_MAX_ABREVIATURA,
  TIPO_CONSUMO_MAX_NOMBRE,
  type TipoConsumo,
  type TipoConsumoInput,
...
/**
 * Mantenimiento de Tipos de consumo.
 *
 ...
 */
@Component({
  selector: 'erp-tipo-consumo-list-page',
  ...
  templateUrl: './tipo-consumo-list-page.html',
  styleUrl: './tipo-consumo-list-page.scss',
})
export class TipoConsumoListPage {
  private readonly api = inject(TipoConsumoApi);
  ...
  protected readonly maxAbreviatura = TIPO_CONSUMO_MAX_ABREVIATURA;
  protected readonly maxNombre = TIPO_CONSUMO_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<TipoConsumo[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de consumo.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoConsumo[]>(() => {
  ...
  protected readonly rowsPagina = computed<TipoConsumo[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoConsumo>[] = [
   ...
   */
  protected async onToggleEstado(row: TipoConsumo): Promise<void> {
  ...
  private patchRow(id: number, patch: Partial<TipoConsumo>): void {
    this.items.update((list) => list.map((it) => (it.tipoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoConsumo[]>([]);

  protected onSeleccion(filas: TipoConsumo[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar tipos de consumo',
   ...
   */
  protected async onReorder(filas: TipoConsumo[]): Promise<void> {
  ...
  protected readonly editando = signal<TipoConsumo | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_CONSUMO_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(TIPO_CONSUMO_MAX_ABREVIATURA)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de consumo' : 'Nuevo tipo de consumo',
  );
  ...
  protected onEditar(row: TipoConsumo): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoConsumoInput = this.form.getRawValue();
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoConsumo): Promise<void> {
    const ok = await this.confirm.ask({
      title: 'Eliminar tipo de consumo',
    ...
    try {
      ...
      // 409 si alguna venta o algún pedido lo usan: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-consumo.csv', matriz);
    } else {
      exportXls('tipos-consumo.xls', matriz);
    }
  }
}
