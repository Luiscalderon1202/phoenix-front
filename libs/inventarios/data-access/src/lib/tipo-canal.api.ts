...
/**
 * Canales de atención (`inventarios.tipocanal`). CRUD completo: el stored procedure del legacy resuelve el
 * alta y la edición enteras en una llamada.
 *
 * `GET /canales-atencion` devuelve el catálogo COMPLETO y sin `meta`: la función del legacy no pagina
...
export class TipoCanalApi {
  private readonly api = inject(ApiService);
  private readonly base = '/canales-atencion';

  /** Catálogo completo, ya ordenado por `orden` desde el backend. */
  list(): Observable<TipoCanal[]> {
    return this.api.get<TipoCanal[]>(this.base);
  }

  get(tipoid: number): Observable<TipoCanal> {
    return this.api.get<TipoCanal>(`${this.base}/${tipoid}`);
  }

  /** Alta. El backend asigna `orden` (al final) y deja el registro activo; no se le mandan. */
  create(input: TipoCanalInput): Observable<TipoCanal> {
    return this.api.post<TipoCanal>(this.base, input);
  }
   ...
   * fuente de verdad.
   */
  update(tipoid: number, input: TipoCanalInput): Observable<TipoCanal> {
    return this.api.put<TipoCanal>(`${this.base}/${tipoid}`, input);
  }

  /** Elimina (204). Falla con 409 si alguna venta lo referencia. */
   ...
   * borra—, así que el saldo se lee de `ResultadoLoteTipoCanal[]`. El backend acota a 200.
   */
  removeLote(ids: readonly number[]): Observable<ResultadoLoteTipoCanal[]> {
    return this.api.post<ResultadoLoteTipoCanal[]>(`${this.base}/lote/eliminar`, { ids });
  }
   ...
   */
  alternarEstado(tipoid: number): Observable<TipoCanalEstado> {
    return this.api.post<TipoCanalEstado>(`${this.base}/${tipoid}/estado/alternar`, {});
  }
