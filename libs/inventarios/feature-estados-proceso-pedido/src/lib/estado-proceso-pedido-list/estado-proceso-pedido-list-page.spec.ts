...
import type { EstadoProcesoPedido, EstadoProcesoPedidoInput } from '@phoenix/inventarios/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { EstadoProcesoPedidoListPage } from './estado-proceso-pedido-list-page';

function fila(over: Partial<EstadoProcesoPedido> = {}): EstadoProcesoPedido {
  return {
    procesoid: 1,
    nombre: 'RECIBIDO',
    abreviatura: 'REC',
...
class EstadoProcesoPedidoApiMock {
  rows: EstadoProcesoPedido[] = [fila(), fila({ procesoid: 2, nombre: 'EN PRODUCCIÓN', abreviatura: 'PROD', orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: EstadoProcesoPedidoInput[] = [];
  readonly actualizados: { id: number; input: EstadoProcesoPedidoInput }[] = [];
  ...
  loteResultado: { procesoid: number; ok: boolean; mensaje?: string }[] | null = null;
  ...
  create(input: EstadoProcesoPedidoInput) {
    this.creados.push(input);
    if (this.fallar) return throwError(() => new Error('409'));
    return of(fila({ procesoid: 99, ...input, orden: this.rows.length + 1 }));
  }

  update(id: number, input: EstadoProcesoPedidoInput) {
    this.actualizados.push({ id, input });
    if (this.fallar) return throwError(() => new Error('409'));
    // El PUT no lleva estado ni orden, así que la respuesta conserva los previos.
    const previo = this.rows.find((row) => row.procesoid === id);
    return of(fila({ ...previo, procesoid: id, ...input }));
  }
  ...
  alternarEstado(procesoid: number) {
    this.alternados.push(procesoid);
    if (this.fallar) return throwError(() => new Error('500'));
    return of({ procesoid, estado: this.estadoResultante });
  }
  ...
  removeLote(ids: readonly number[]) {
    ...
    if (this.fallar) return throwError(() => new Error('500'));
    return of(this.loteResultado ?? ids.map((procesoid) => ({ procesoid, ok: true })));
  }
}
...
async function setup(api = new EstadoProcesoPedidoApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [EstadoProcesoPedidoListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: EstadoProcesoPedidoApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(EstadoProcesoPedidoListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): EstadoProcesoPedidoInput };
    rows(): EstadoProcesoPedido[];
    filtrando(): boolean;
    modalAbierto(): boolean;
    tituloModal(): string;
    seleccionadas(): { procesoid: number }[];
    onExportar(e: { formato: string; detalle: boolean }): void;
    onCerrar(): void;
    onPageSize(n: number): void;
    onPage(n: number): void;
    rowsPagina(): EstadoProcesoPedido[];
...
describe('EstadoProcesoPedidoListPage', () => {
  ...
  it('pinta el catálogo que devuelve el backend', async () => {
    ...
    expect(celdas(el, 1)).toEqual(['RECIBIDO', 'EN PRODUCCIÓN']);
  });
  ...
  it('filtra en cliente por texto: no hay endpoint al que mandar el filtro', async () => {
    ...
    c.filters.patchValue({ q: 'en' });
    c.onApply();
    fixture.detectChanges();

    expect(c.rows().map((row) => row.procesoid)).toEqual([2]);
  });

  it('filtra por estado', async () => {
    const api = new EstadoProcesoPedidoApiMock();
    api.rows = [fila(), fila({ procesoid: 2, estado: false })];
  ...
  it('al reordenar manda la lista completa desde la posición 1 y renumera en local', async () => {
    ...
    expect(c.rows().map((row) => [row.procesoid, row.orden])).toEqual([
      [2, 1],
      [1, 2],
    ]);
  });
  ...
  it('revierte el orden si el backend falla', async () => {
    ...
    expect(c.rows().map((row) => row.procesoid)).toEqual([1, 2]);
  });
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'ENTREGADO', abreviatura: 'ENT' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'ENTREGADO', abreviatura: 'ENT' }]);
  ...
  it('el alta añade la fila al final, que es donde la coloca el backend', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'ENTREGADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.rows().map((row) => row.nombre).at(-1)).toBe('ENTREGADO');
  });
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'RECIBIDO' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('RECIBIDO');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar estado de proceso');
    expect(c.form.getRawValue()).toEqual({ nombre: 'RECIBIDO', abreviatura: 'REC' });

    c.form.patchValue({ nombre: 'RECIBIDO EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('RECIBIDO EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new EstadoProcesoPedidoApiMock();
    api.rows = [fila(), fila({ procesoid: 2, nombre: 'EN PRODUCCIÓN', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'EN PRODUCCIÓN EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados[0].input).not.toHaveProperty('estado');
    expect(c.rows().map((row) => [row.procesoid, row.estado])).toEqual([
      [1, true],
      [2, false],
    ]);
  });
  ...
  it('el borrado en lote va en UNA petición y quita las filas', async () => {
    ...
    c.onSeleccion([{ procesoid: 1 }, { procesoid: 2 }] as never);
  ...
  it('el lote es parcial: conserva las filas que fallaron', async () => {
    ...
    api.loteResultado = [
      { procesoid: 1, ok: true },
      { procesoid: 2, ok: false, mensaje: 'Tiene registros relacionados.' },
    ];
  ...
  it('el borrado en lote pide confirmación', async () => {
    ...
    c.onSeleccion([{ procesoid: 1 }] as never);
    c.onEliminarSeleccionadas();
    await asentar(fixture);
  ...
  it('pagina en cliente: el endpoint devuelve el catálogo entero', async () => {
    ...
    api.rows = Array.from({ length: 30 }, (_, i) =>
      fila({ procesoid: i + 1, nombre: `TIPO ${i + 1}`, orden: i + 1 }),
    );
