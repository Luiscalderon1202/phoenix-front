...
import { EstadoProcesoPedidoApi } from '@phoenix/inventarios/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  ESTADO_PROCESO_PEDIDO_MAX_ABREVIATURA,
  ESTADO_PROCESO_PEDIDO_MAX_NOMBRE,
  type EstadoProcesoPedido,
  type EstadoProcesoPedidoInput,
...
/**
 * Mantenimiento de Estados de proceso.
 *
 ...
 * tiene.
 * ⚠ La clave de este catálogo es `procesoid`, no `tipoid`.
 */
@Component({
  selector: 'erp-estado-proceso-pedido-list-page',
  ...
  templateUrl: './estado-proceso-pedido-list-page.html',
  styleUrl: './estado-proceso-pedido-list-page.scss',
})
export class EstadoProcesoPedidoListPage {
  private readonly api = inject(EstadoProcesoPedidoApi);
  ...
  protected readonly maxAbreviatura = ESTADO_PROCESO_PEDIDO_MAX_ABREVIATURA;
  protected readonly maxNombre = ESTADO_PROCESO_PEDIDO_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<EstadoProcesoPedido[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de estados de proceso.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<EstadoProcesoPedido[]>(() => {
  ...
  protected readonly rowsPagina = computed<EstadoProcesoPedido[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<EstadoProcesoPedido>[] = [
   ...
   */
  protected async onToggleEstado(row: EstadoProcesoPedido): Promise<void> {
    this.patchRow(row.procesoid, { estado: !row.estado });
    try {
      const resultado = await firstValueFrom(this.api.alternarEstado(row.procesoid));
      if (typeof resultado?.estado === 'boolean') {
        this.patchRow(row.procesoid, { estado: resultado.estado });
      }
    } catch {
      this.patchRow(row.procesoid, { estado: row.estado });
    }
  ...
  private patchRow(id: number, patch: Partial<EstadoProcesoPedido>): void {
    this.items.update((list) => list.map((it) => (it.procesoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<EstadoProcesoPedido[]>([]);

  protected onSeleccion(filas: EstadoProcesoPedido[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar estados de proceso',
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.procesoid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) {
        const borrados = new Set(resultados.filter((res) => res.ok).map((res) => res.procesoid));
        this.items.update((list) => list.filter((it) => !borrados.has(it.procesoid)));
   ...
   */
  protected async onReorder(filas: EstadoProcesoPedido[]): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.reordenar(filas.map((f) => f.procesoid), 1));
    } catch {
  ...
  protected readonly editando = signal<EstadoProcesoPedido | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(ESTADO_PROCESO_PEDIDO_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(ESTADO_PROCESO_PEDIDO_MAX_ABREVIATURA)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar estado de proceso' : 'Nuevo estado de proceso',
  );
  ...
  protected onEditar(row: EstadoProcesoPedido): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: EstadoProcesoPedidoInput = this.form.getRawValue();
    const enEdicion = this.editando();
    this.guardando.set(true);

    try {
      if (enEdicion) {
        const actualizado = await firstValueFrom(this.api.update(enEdicion.procesoid, input));
        this.patchRow(enEdicion.procesoid, actualizado);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: EstadoProcesoPedido): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      await firstValueFrom(this.api.remove(row.procesoid));
      this.items.update((list) => list.filter((it) => it.procesoid !== row.procesoid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.procesoid !== row.procesoid));
      this.notify.success(`Se eliminó "${row.nombre}".`);
    } catch {
      // 409 si hay algún pedido en ese estado: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    ...
    if (e.formato === 'csv') {
      exportCsv('estados-proceso-pedido.csv', matriz);
    } else {
      exportXls('estados-proceso-pedido.xls', matriz);
    }
  }
}
