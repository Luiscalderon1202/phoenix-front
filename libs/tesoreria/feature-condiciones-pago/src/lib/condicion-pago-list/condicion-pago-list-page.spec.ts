...
import { CondicionPagoApi } from '@phoenix/tesoreria/data-access';
import type { CondicionPago, CondicionPagoInput } from '@phoenix/tesoreria/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { CondicionPagoListPage } from './condicion-pago-list-page';

function fila(over: Partial<CondicionPago> = {}): CondicionPago {
  return {
    condicionpagoid: 1,
    nombre: 'CONTADO',
    abreviatura: 'CON',
...
class CondicionPagoApiMock {
  rows: CondicionPago[] = [fila(), fila({ condicionpagoid: 2, nombre: 'CREDITO 30', abreviatura: 'C30', cantidad: 30, orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: CondicionPagoInput[] = [];
  readonly actualizados: { id: number; input: CondicionPagoInput }[] = [];
  ...
  loteResultado: { condicionpagoid: number; ok: boolean; mensaje?: string }[] | null = null;
  ...
  create(input: CondicionPagoInput) {
    this.creados.push(input);
    if (this.fallar) return throwError(() => new Error('409'));
    return of(fila({ condicionpagoid: 99, ...input, orden: this.rows.length + 1 }));
  }

  update(id: number, input: CondicionPagoInput) {
    this.actualizados.push({ id, input });
    if (this.fallar) return throwError(() => new Error('409'));
    // El PUT no lleva estado ni orden, así que la respuesta conserva los previos.
    const previo = this.rows.find((row) => row.condicionpagoid === id);
    return of(fila({ ...previo, condicionpagoid: id, ...input }));
  }
  ...
  alternarEstado(condicionpagoid: number) {
    this.alternados.push(condicionpagoid);
    if (this.fallar) return throwError(() => new Error('500'));
    return of({ condicionpagoid, estado: this.estadoResultante });
  }
  ...
  removeLote(ids: readonly number[]) {
    ...
    if (this.fallar) return throwError(() => new Error('500'));
    return of(this.loteResultado ?? ids.map((condicionpagoid) => ({ condicionpagoid, ok: true })));
  }
}
...
async function setup(api = new CondicionPagoApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [CondicionPagoListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: CondicionPagoApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(CondicionPagoListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): CondicionPagoInput };
    rows(): CondicionPago[];
    filtrando(): boolean;
    modalAbierto(): boolean;
    tituloModal(): string;
    seleccionadas(): { condicionpagoid: number }[];
    onExportar(e: { formato: string; detalle: boolean }): void;
    onCerrar(): void;
    onPageSize(n: number): void;
    onPage(n: number): void;
    rowsPagina(): CondicionPago[];
...
describe('CondicionPagoListPage', () => {
  ...
  it('filtra en cliente por texto: no hay endpoint al que mandar el filtro', async () => {
    ...
    c.filters.patchValue({ q: 'credito' });
    c.onApply();
    fixture.detectChanges();

    expect(c.rows().map((row) => row.condicionpagoid)).toEqual([2]);
  });

  it('filtra por estado', async () => {
    const api = new CondicionPagoApiMock();
    api.rows = [fila(), fila({ condicionpagoid: 2, estado: false })];
  ...
  it('al reordenar manda la lista completa desde la posición 1 y renumera en local', async () => {
    ...
    expect(c.rows().map((row) => [row.condicionpagoid, row.orden])).toEqual([
      [2, 1],
      [1, 2],
    ]);
  });
  ...
  it('revierte el orden si el backend falla', async () => {
    ...
    expect(c.rows().map((row) => row.condicionpagoid)).toEqual([1, 2]);
  });
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'CREDITO 45', abreviatura: 'C45', cantidad: 45 });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'CREDITO 45', abreviatura: 'C45', cantidad: 45 }]);
  ...
  it('el formulario no ofrece ni orden ni estado', async () => {
    ...
    expect(c.form.getRawValue()).toEqual({ nombre: '', abreviatura: '', cantidad: 0 });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar condición de pago');
    expect(c.form.getRawValue()).toEqual({ nombre: 'CONTADO', abreviatura: 'CON', cantidad: 0 });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new CondicionPagoApiMock();
    api.rows = [fila(), fila({ condicionpagoid: 2, nombre: 'CREDITO 30', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'CREDITO 30 EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados[0].input).not.toHaveProperty('estado');
    expect(c.rows().map((row) => [row.condicionpagoid, row.estado])).toEqual([
      [1, true],
      [2, false],
    ]);
  });
  ...
  it('el borrado en lote va en UNA petición y quita las filas', async () => {
    ...
    c.onSeleccion([{ condicionpagoid: 1 }, { condicionpagoid: 2 }] as never);
  ...
  it('el lote es parcial: conserva las filas que fallaron', async () => {
    const api = new CondicionPagoApiMock();
    api.loteResultado = [
      { condicionpagoid: 1, ok: true },
      { condicionpagoid: 2, ok: false, mensaje: 'Tiene registros relacionados.' },
    ];
  ...
  it('el borrado en lote pide confirmación', async () => {
    ...
    c.onSeleccion([{ condicionpagoid: 1 }] as never);
    c.onEliminarSeleccionadas();
    await asentar(fixture);
  ...
  it('pagina en cliente: el endpoint devuelve el catálogo entero', async () => {
    ...
    api.rows = Array.from({ length: 30 }, (_, i) =>
      fila({ condicionpagoid: i + 1, nombre: `FILA ${i + 1}`, orden: i + 1 }),
    );
  ...
  it('los días de plazo viajan en el cuerpo y se pintan en la grilla', async () => {
    // Es el campo que el pedido copia en su `diasplazo`.
    const { fixture, api, el } = await setup();
    const c = comp(fixture);

    expect(celdas(el, 3)).toEqual(['0', '30']);

    c.onEditar(fila() as never);
    c.form.patchValue({ cantidad: 15 });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados[0].input.cantidad).toBe(15);
  });
});
