...
import {
  CONDICION_PAGO_MAX_ABREVIATURA,
  CONDICION_PAGO_MAX_NOMBRE,
  type CondicionPago,
 ...
 */
@Component({
  selector: 'erp-condicion-pago-list-page',
  ...
  templateUrl: './condicion-pago-list-page.html',
  styleUrl: './condicion-pago-list-page.scss',
})
export class CondicionPagoListPage {
  private readonly api = inject(CondicionPagoApi);
  ...
  protected readonly maxAbreviatura = CONDICION_PAGO_MAX_ABREVIATURA;
  protected readonly maxNombre = CONDICION_PAGO_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<CondicionPago[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de condiciones de pago.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<CondicionPago[]>(() => {
  ...
  protected readonly rowsPagina = computed<CondicionPago[]>(() => {
   ...
   */
  protected async onToggleEstado(row: CondicionPago): Promise<void> {
    this.patchRow(row.condicionpagoid, { estado: !row.estado });
    try {
      const resultado = await firstValueFrom(this.api.alternarEstado(row.condicionpagoid));
      if (typeof resultado?.estado === 'boolean') {
        this.patchRow(row.condicionpagoid, { estado: resultado.estado });
      }
    } catch {
      this.patchRow(row.condicionpagoid, { estado: row.estado });
    }
  ...
  private patchRow(id: number, patch: Partial<CondicionPago>): void {
    this.items.update((list) => list.map((it) => (it.condicionpagoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<CondicionPago[]>([]);

  protected onSeleccion(filas: CondicionPago[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.condicionpagoid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) {
        const borrados = new Set(resultados.filter((res) => res.ok).map((res) => res.condicionpagoid));
        this.items.update((list) => list.filter((it) => !borrados.has(it.condicionpagoid)));
   ...
   */
  protected async onReorder(filas: CondicionPago[]): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.reordenar(filas.map((f) => f.condicionpagoid), 1));
    } catch {
  ...
  protected readonly editando = signal<CondicionPago | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(CONDICION_PAGO_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(CONDICION_PAGO_MAX_ABREVIATURA)]],
    cantidad: [0, [Validators.min(0)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar condición de pago' : 'Nuevo condición de pago',
  );

  protected onNuevo(): void {
    this.editando.set(null);
    this.form.reset({ nombre: '', abreviatura: '', cantidad: 0 });
    this.modalAbierto.set(true);
  }

  protected onEditar(row: CondicionPago): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: CondicionPagoInput = this.form.getRawValue();
    const enEdicion = this.editando();
    this.guardando.set(true);

    try {
      if (enEdicion) {
        const actualizado = await firstValueFrom(this.api.update(enEdicion.condicionpagoid, input));
        this.patchRow(enEdicion.condicionpagoid, actualizado);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: CondicionPago): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      await firstValueFrom(this.api.remove(row.condicionpagoid));
      this.items.update((list) => list.filter((it) => it.condicionpagoid !== row.condicionpagoid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.condicionpagoid !== row.condicionpagoid));
      this.notify.success(`Se eliminó "${row.nombre}".`);
    } catch {
      // 409 si alguna venta u orden de compra la usa: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    const cabeceras = ['Orden', 'ID', 'Nombre', 'Abreviatura', 'Días', 'Estado'];
    const matriz = [
      cabeceras,
      ...this.rows().map((t) => [
        t.orden,
    ...
    if (e.formato === 'csv') {
      exportCsv('condiciones-pago.csv', matriz);
    } else {
      exportXls('condiciones-pago.xls', matriz);
    }
  }
}
