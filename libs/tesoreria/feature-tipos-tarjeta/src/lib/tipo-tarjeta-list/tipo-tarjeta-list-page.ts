...
import { TipoTarjetaApi } from '@phoenix/tesoreria/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  TIPO_TARJETA_MAX_ABREVIATURA,
  TIPO_TARJETA_MAX_NOMBRE,
  type TipoTarjeta,
  type TipoTarjetaInput,
...
/**
 * Mantenimiento de Tipos de tarjeta.
 *
 ...
 */
@Component({
  selector: 'erp-tipo-tarjeta-list-page',
  ...
  templateUrl: './tipo-tarjeta-list-page.html',
  styleUrl: './tipo-tarjeta-list-page.scss',
})
export class TipoTarjetaListPage {
  private readonly api = inject(TipoTarjetaApi);
  ...
  protected readonly maxAbreviatura = TIPO_TARJETA_MAX_ABREVIATURA;
  protected readonly maxNombre = TIPO_TARJETA_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<TipoTarjeta[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de tarjeta.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoTarjeta[]>(() => {
  ...
  protected readonly rowsPagina = computed<TipoTarjeta[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoTarjeta>[] = [
   ...
   */
  protected async onToggleEstado(row: TipoTarjeta): Promise<void> {
    this.patchRow(row.tipotarjetaid, { estado: !row.estado });
    try {
      const resultado = await firstValueFrom(this.api.alternarEstado(row.tipotarjetaid));
      if (typeof resultado?.estado === 'boolean') {
        this.patchRow(row.tipotarjetaid, { estado: resultado.estado });
      }
    } catch {
      this.patchRow(row.tipotarjetaid, { estado: row.estado });
    }
  ...
  private patchRow(id: number, patch: Partial<TipoTarjeta>): void {
    this.items.update((list) => list.map((it) => (it.tipotarjetaid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoTarjeta[]>([]);

  protected onSeleccion(filas: TipoTarjeta[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar tipos de tarjeta',
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.tipotarjetaid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) {
        const borrados = new Set(resultados.filter((res) => res.ok).map((res) => res.tipotarjetaid));
        this.items.update((list) => list.filter((it) => !borrados.has(it.tipotarjetaid)));
   ...
   */
  protected async onReorder(filas: TipoTarjeta[]): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.reordenar(filas.map((f) => f.tipotarjetaid), 1));
    } catch {
  ...
  protected readonly editando = signal<TipoTarjeta | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_TARJETA_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(TIPO_TARJETA_MAX_ABREVIATURA)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de tarjeta' : 'Nuevo tipo de tarjeta',
  );
  ...
  protected onEditar(row: TipoTarjeta): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoTarjetaInput = this.form.getRawValue();
    const enEdicion = this.editando();
    this.guardando.set(true);

    try {
      if (enEdicion) {
        const actualizado = await firstValueFrom(this.api.update(enEdicion.tipotarjetaid, input));
        this.patchRow(enEdicion.tipotarjetaid, actualizado);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoTarjeta): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      await firstValueFrom(this.api.remove(row.tipotarjetaid));
      this.items.update((list) => list.filter((it) => it.tipotarjetaid !== row.tipotarjetaid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.tipotarjetaid !== row.tipotarjetaid));
      this.notify.success(`Se eliminó "${row.nombre}".`);
    } catch {
      // 409 si alguna forma de pago de una venta lo referencia: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-tarjeta.csv', matriz);
    } else {
      exportXls('tipos-tarjeta.xls', matriz);
    }
  }
}
