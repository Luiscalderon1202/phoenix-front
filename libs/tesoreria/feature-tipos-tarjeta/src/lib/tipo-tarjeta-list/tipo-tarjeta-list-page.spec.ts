...
import type { TipoTarjeta, TipoTarjetaInput } from '@phoenix/tesoreria/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { TipoTarjetaListPage } from './tipo-tarjeta-list-page';

function fila(over: Partial<TipoTarjeta> = {}): TipoTarjeta {
  return {
    tipotarjetaid: 1,
    nombre: 'VISA',
    abreviatura: 'VIS',
...
class TipoTarjetaApiMock {
  rows: TipoTarjeta[] = [fila(), fila({ tipotarjetaid: 2, nombre: 'MASTERCARD', abreviatura: 'MAS', orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: TipoTarjetaInput[] = [];
  readonly actualizados: { id: number; input: TipoTarjetaInput }[] = [];
  ...
  loteResultado: { tipotarjetaid: number; ok: boolean; mensaje?: string }[] | null = null;
  ...
  create(input: TipoTarjetaInput) {
    this.creados.push(input);
    if (this.fallar) return throwError(() => new Error('409'));
    return of(fila({ tipotarjetaid: 99, ...input, orden: this.rows.length + 1 }));
  }

  update(id: number, input: TipoTarjetaInput) {
    this.actualizados.push({ id, input });
    if (this.fallar) return throwError(() => new Error('409'));
    // El PUT no lleva estado ni orden, así que la respuesta conserva los previos.
    const previo = this.rows.find((row) => row.tipotarjetaid === id);
    return of(fila({ ...previo, tipotarjetaid: id, ...input }));
  }
  ...
  alternarEstado(tipotarjetaid: number) {
    this.alternados.push(tipotarjetaid);
    if (this.fallar) return throwError(() => new Error('500'));
    return of({ tipotarjetaid, estado: this.estadoResultante });
  }
  ...
  removeLote(ids: readonly number[]) {
    ...
    if (this.fallar) return throwError(() => new Error('500'));
    return of(this.loteResultado ?? ids.map((tipotarjetaid) => ({ tipotarjetaid, ok: true })));
  }
}
...
async function setup(api = new TipoTarjetaApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [TipoTarjetaListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: TipoTarjetaApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(TipoTarjetaListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): TipoTarjetaInput };
    rows(): TipoTarjeta[];
    filtrando(): boolean;
    modalAbierto(): boolean;
    tituloModal(): string;
    seleccionadas(): { tipotarjetaid: number }[];
    onExportar(e: { formato: string; detalle: boolean }): void;
    onCerrar(): void;
    onPageSize(n: number): void;
    onPage(n: number): void;
    rowsPagina(): TipoTarjeta[];
...
describe('TipoTarjetaListPage', () => {
  ...
  it('filtra en cliente por texto: no hay endpoint al que mandar el filtro', async () => {
    ...
    c.filters.patchValue({ q: 'master' });
    c.onApply();
    fixture.detectChanges();

    expect(c.rows().map((row) => row.tipotarjetaid)).toEqual([2]);
  });

  it('filtra por estado', async () => {
    const api = new TipoTarjetaApiMock();
    api.rows = [fila(), fila({ tipotarjetaid: 2, estado: false })];
  ...
  it('al reordenar manda la lista completa desde la posición 1 y renumera en local', async () => {
    ...
    expect(c.rows().map((row) => [row.tipotarjetaid, row.orden])).toEqual([
      [2, 1],
      [1, 2],
    ]);
  });
  ...
  it('revierte el orden si el backend falla', async () => {
    ...
    expect(c.rows().map((row) => row.tipotarjetaid)).toEqual([1, 2]);
  });
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'AMERICAN EXPRESS', abreviatura: 'AMX' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'AMERICAN EXPRESS', abreviatura: 'AMX' }]);
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'VISA' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('VISA');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar tipo de tarjeta');
    expect(c.form.getRawValue()).toEqual({ nombre: 'VISA', abreviatura: 'VIS' });

    c.form.patchValue({ nombre: 'VISA EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('VISA EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new TipoTarjetaApiMock();
    api.rows = [fila(), fila({ tipotarjetaid: 2, nombre: 'MASTERCARD', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'MASTERCARD EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados[0].input).not.toHaveProperty('estado');
    expect(c.rows().map((row) => [row.tipotarjetaid, row.estado])).toEqual([
      [1, true],
      [2, false],
    ]);
  });
  ...
  it('el borrado en lote va en UNA petición y quita las filas', async () => {
    ...
    c.onSeleccion([{ tipotarjetaid: 1 }, { tipotarjetaid: 2 }] as never);
  ...
  it('el lote es parcial: conserva las filas que fallaron', async () => {
    const api = new TipoTarjetaApiMock();
    api.loteResultado = [
      { tipotarjetaid: 1, ok: true },
      { tipotarjetaid: 2, ok: false, mensaje: 'Tiene registros relacionados.' },
    ];
  ...
  it('el borrado en lote pide confirmación', async () => {
    ...
    c.onSeleccion([{ tipotarjetaid: 1 }] as never);
    c.onEliminarSeleccionadas();
    await asentar(fixture);
  ...
  it('pagina en cliente: el endpoint devuelve el catálogo entero', async () => {
    ...
    api.rows = Array.from({ length: 30 }, (_, i) =>
      fila({ tipotarjetaid: i + 1, nombre: `FILA ${i + 1}`, orden: i + 1 }),
    );
