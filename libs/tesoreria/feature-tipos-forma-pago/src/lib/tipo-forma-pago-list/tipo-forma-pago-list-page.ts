...
import {
  TIPO_FORMA_PAGO_MAX_ABREVIATURA,
  TIPO_FORMA_PAGO_MAX_ESTRUCTURA,
  TIPO_FORMA_PAGO_MAX_NOMBRE,
  TIPO_FORMA_PAGO_MAX_PCGR,
  TIPO_FORMA_PAGO_MAX_TIPO,
  type TipoFormaPago,
  type TipoFormaPagoInput,
 ...
 */
@Component({
  selector: 'erp-tipo-forma-pago-list-page',
  ...
  templateUrl: './tipo-forma-pago-list-page.html',
  styleUrl: './tipo-forma-pago-list-page.scss',
})
export class TipoFormaPagoListPage {
  private readonly api = inject(TipoFormaPagoApi);
  ...
  protected readonly maxAbreviatura = TIPO_FORMA_PAGO_MAX_ABREVIATURA;
  protected readonly maxEstructura = TIPO_FORMA_PAGO_MAX_ESTRUCTURA;
  protected readonly maxNombre = TIPO_FORMA_PAGO_MAX_NOMBRE;
  protected readonly maxPcgr = TIPO_FORMA_PAGO_MAX_PCGR;
  protected readonly maxTipo = TIPO_FORMA_PAGO_MAX_TIPO;
   ...
   */
  private readonly items = linkedSignal<TipoFormaPago[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de forma de pago.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoFormaPago[]>(() => {
    ...
    return this.items().filter((t) => {
      ...
      return t.nombre.toLocaleLowerCase('es').includes(texto) || t.abreviatura.toLocaleLowerCase('es').includes(texto) || t.estructura.toLocaleLowerCase('es').includes(texto) || t.tipo.toLocaleLowerCase('es').includes(texto) || t.pcgr_general.toLocaleLowerCase('es').includes(texto) || t.pcgr_empresarial.toLocaleLowerCase('es').includes(texto);
    });
  });
  ...
  protected readonly rowsPagina = computed<TipoFormaPago[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoFormaPago>[] = [
    { key: 'orden', header: '#', width: '60px', align: 'right', mono: true },
    { key: 'nombre', header: 'Nombre' },
    { key: 'abreviatura', header: 'Abreviatura', width: '140px' },
    { key: 'estructura', header: 'Estructura', width: '120px' },
    { key: 'tipo', header: 'Tipo', width: '110px' },
    { key: 'pcgr_general', header: 'PCGR general', width: '130px' },
    { key: 'pcgr_empresarial', header: 'PCGR empresarial', width: '150px' },
   ...
   */
  protected async onToggleEstado(row: TipoFormaPago): Promise<void> {
    this.patchRow(row.tipoformapagoid, { estado: !row.estado });
    try {
      const resultado = await firstValueFrom(this.api.alternarEstado(row.tipoformapagoid));
      if (typeof resultado?.estado === 'boolean') {
        this.patchRow(row.tipoformapagoid, { estado: resultado.estado });
      }
    } catch {
      this.patchRow(row.tipoformapagoid, { estado: row.estado });
    }
  ...
  private patchRow(id: number, patch: Partial<TipoFormaPago>): void {
    this.items.update((list) => list.map((it) => (it.tipoformapagoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoFormaPago[]>([]);

  protected onSeleccion(filas: TipoFormaPago[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar tipos de forma de pago',
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.tipoformapagoid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) {
        const borrados = new Set(resultados.filter((res) => res.ok).map((res) => res.tipoformapagoid));
        this.items.update((list) => list.filter((it) => !borrados.has(it.tipoformapagoid)));
   ...
   */
  protected async onReorder(filas: TipoFormaPago[]): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.reordenar(filas.map((f) => f.tipoformapagoid), 1));
    } catch {
  ...
  protected readonly editando = signal<TipoFormaPago | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_FORMA_PAGO_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(TIPO_FORMA_PAGO_MAX_ABREVIATURA)]],
    estructura: ['', [Validators.maxLength(TIPO_FORMA_PAGO_MAX_ESTRUCTURA)]],
    tipo: ['', [Validators.maxLength(TIPO_FORMA_PAGO_MAX_TIPO)]],
    pcgr_general: ['', [Validators.maxLength(TIPO_FORMA_PAGO_MAX_PCGR)]],
    pcgr_empresarial: ['', [Validators.maxLength(TIPO_FORMA_PAGO_MAX_PCGR)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de forma de pago' : 'Nuevo tipo de forma de pago',
  );

  protected onNuevo(): void {
    this.editando.set(null);
    this.form.reset({ nombre: '', abreviatura: '', estructura: '', tipo: '', pcgr_general: '', pcgr_empresarial: '' });
    this.modalAbierto.set(true);
  }

  protected onEditar(row: TipoFormaPago): void {
    ...
    this.form.reset({
      ...
      tipo: row.tipo,
      pcgr_general: row.pcgr_general,
      pcgr_empresarial: row.pcgr_empresarial,
    });
    this.modalAbierto.set(true);
  }
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoFormaPagoInput = this.form.getRawValue();
    const enEdicion = this.editando();
    this.guardando.set(true);

    try {
      if (enEdicion) {
        const actualizado = await firstValueFrom(this.api.update(enEdicion.tipoformapagoid, input));
        this.patchRow(enEdicion.tipoformapagoid, actualizado);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoFormaPago): Promise<void> {
    const ok = await this.confirm.ask({
      title: 'Eliminar tipo de forma de pago',
    ...
    try {
      await firstValueFrom(this.api.remove(row.tipoformapagoid));
      this.items.update((list) => list.filter((it) => it.tipoformapagoid !== row.tipoformapagoid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.tipoformapagoid !== row.tipoformapagoid));
      this.notify.success(`Se eliminó "${row.nombre}".`);
    } catch {
      // 409 si lo usa una venta, un almacén o un detalle de caja: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    const cabeceras = ['Orden', 'ID', 'Nombre', 'Abreviatura', 'Estructura', 'Tipo', 'PCGR general', 'PCGR empresarial', 'Estado'];
    const matriz = [
      cabeceras,
      ...this.rows().map((t) => [
        t.orden,
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-forma-pago.csv', matriz);
    } else {
      exportXls('tipos-forma-pago.xls', matriz);
    }
  }
}
