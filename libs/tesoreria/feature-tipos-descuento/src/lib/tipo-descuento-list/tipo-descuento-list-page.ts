...
import { TipoDescuentoApi } from '@phoenix/tesoreria/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  TIPO_DESCUENTO_MAX_ABREVIATURA,
  TIPO_DESCUENTO_MAX_NOMBRE,
  type TipoDescuento,
...
/**
 * Mantenimiento de Tipos de descuento.
 *
 ...
 * tiene.
 * ⚠ El borrado de este catálogo no está protegido por nada: su stored procedure no comprueba
 * relaciones y contra la tabla no hay ninguna clave foránea. Eliminar un tipo en uso deja ids
 * colgados y la API no puede impedirlo.
 */
@Component({
  selector: 'erp-tipo-descuento-list-page',
  ...
  templateUrl: './tipo-descuento-list-page.html',
  styleUrl: './tipo-descuento-list-page.scss',
})
export class TipoDescuentoListPage {
  private readonly api = inject(TipoDescuentoApi);
  ...
  protected readonly maxAbreviatura = TIPO_DESCUENTO_MAX_ABREVIATURA;
  protected readonly maxNombre = TIPO_DESCUENTO_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<TipoDescuento[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de descuento.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoDescuento[]>(() => {
  ...
  protected readonly rowsPagina = computed<TipoDescuento[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoDescuento>[] = [
   ...
   */
  protected async onToggleEstado(row: TipoDescuento): Promise<void> {
  ...
  private patchRow(id: number, patch: Partial<TipoDescuento>): void {
    this.items.update((list) => list.map((it) => (it.tipoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoDescuento[]>([]);

  protected onSeleccion(filas: TipoDescuento[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar tipos de descuento',
   ...
   */
  protected async onReorder(filas: TipoDescuento[]): Promise<void> {
  ...
  protected readonly editando = signal<TipoDescuento | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_DESCUENTO_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(TIPO_DESCUENTO_MAX_ABREVIATURA)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de descuento' : 'Nuevo tipo de descuento',
  );
  ...
  protected onEditar(row: TipoDescuento): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoDescuentoInput = this.form.getRawValue();
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoDescuento): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      ...
      // 409 si hay registros relacionados: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-descuento.csv', matriz);
    } else {
      exportXls('tipos-descuento.xls', matriz);
    }
  }
}
