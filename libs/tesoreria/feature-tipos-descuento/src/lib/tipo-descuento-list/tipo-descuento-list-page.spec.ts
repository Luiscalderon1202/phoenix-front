...
import type { TipoDescuento, TipoDescuentoInput } from '@phoenix/tesoreria/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { TipoDescuentoListPage } from './tipo-descuento-list-page';

function fila(over: Partial<TipoDescuento> = {}): TipoDescuento {
  return {
    tipoid: 1,
    nombre: 'PROMOCION',
    abreviatura: 'PRO',
...
class TipoDescuentoApiMock {
  rows: TipoDescuento[] = [fila(), fila({ tipoid: 2, nombre: 'FIDELIZACION', abreviatura: 'FID', orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: TipoDescuentoInput[] = [];
  readonly actualizados: { id: number; input: TipoDescuentoInput }[] = [];
  ...
  create(input: TipoDescuentoInput) {
  ...
  update(id: number, input: TipoDescuentoInput) {
...
async function setup(api = new TipoDescuentoApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [TipoDescuentoListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: TipoDescuentoApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(TipoDescuentoListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): TipoDescuentoInput };
    rows(): TipoDescuento[];
    filtrando(): boolean;
...
describe('TipoDescuentoListPage', () => {
  ...
  it('filtra por estado', async () => {
    const api = new TipoDescuentoApiMock();
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'LIQUIDACION', abreviatura: 'LIQ' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'LIQUIDACION', abreviatura: 'LIQ' }]);
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'PROMOCION' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('PROMOCION');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar tipo de descuento');
    expect(c.form.getRawValue()).toEqual({ nombre: 'PROMOCION', abreviatura: 'PRO' });

    c.form.patchValue({ nombre: 'PROMOCION EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('PROMOCION EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new TipoDescuentoApiMock();
    api.rows = [fila(), fila({ tipoid: 2, nombre: 'FIDELIZACION', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'FIDELIZACION EDITADO' });
    c.onGuardar();
    await asentar(fixture);
  ...
  it('pagina en cliente: el endpoint devuelve el catálogo entero', async () => {
    ...
    api.rows = Array.from({ length: 30 }, (_, i) =>
      fila({ tipoid: i + 1, nombre: `FILA ${i + 1}`, orden: i + 1 }),
    );
