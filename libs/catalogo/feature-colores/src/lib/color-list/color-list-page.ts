...
/**
 * Mantenimiento de Colores de producto (`catalogo.color`, proceso `CAT-COLOR`).
 *
 ...
 * el proceso que el `.php` ya declaraba y no con uno inventado, para que el permiso concedido
 * valga en las dos aplicaciones mientras convivan.
 *
 * ⚠ LA COLUMNA `color` DE LA TABLA NO SE PINTA NI SE EDITA, y no es un olvido. Está muerta en
 * las tres capas del legacy: `ColorEdit.php` no tiene input para ella, `color.js:29` pinta esa
 * celda del listado como `<td></td>` —siempre vacía— y `ajColor.php:31` manda `txtColor`, un
 * campo que el formulario nunca envía, o sea la cadena vacía. **Cada guardado del legacy la
 * borra**, y por eso las dos filas de producción la tienen vacía. El backend la PRESERVA sin
 * exponerla: relee la fila y le devuelve al stored procedure el valor que ya había. Si algún
 * día se le da pantalla, el dato seguirá ahí.
 *
 * A diferencia de líneas y grupos, aquí el reordenamiento es del LEGACY:
 * `catalogo.pacolor_cambiar_orden` ya recibe `vstart` y ya numera `orden = posición +
 * (vstart-1)`. Es el único recurso del tramo cuya familia de funciones está completa.
 *
 * Repartos de trabajo entre servidor y cliente, que aquí no coinciden:
 *
 * - **El filtro por nombre va al SERVIDOR** (`catalogo.pacolor_leer` lo recibe). Filtrar aquí
 ...
 * - **La paginación va al CLIENTE**: el endpoint devuelve el catálogo entero y sin `meta`.
 *   Existe `pacolor_count`, pero sin un `_leer` paginado con el que emparejarse.
 *
 * Y esta pantalla NO cuelga del hub de Tablas Básicas, así que cerrar vuelve a `/inicio`.
 */
@Component({
  selector: 'erp-color-list-page',
...
export class ColorListPage {
  private readonly api = inject(ColorApi);
  ...
  protected readonly maxNombre = COLOR_MAX_NOMBRE;
  protected readonly maxAbreviatura = COLOR_MAX_ABREVIATURA;
   ...
   * lista.
   */
  private readonly items = linkedSignal<Color[]>(() => this.catalogo.value() ?? []);

  /** Filas visibles: lo que trajo el servidor, con el filtro de estado aplicado en cliente. */
  protected readonly rows = computed<Color[]>(() => {
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error ? e.message : e ? 'No se pudo cargar el catálogo de colores.' : null;
  });
  ...
  protected readonly rowsPagina = computed<Color[]>(() => {
  ...
  protected readonly columns: GridColumn<Color>[] = [
   ...
   */
  protected async onToggleEstado(row: Color): Promise<void> {
    this.patchRow(row.colorid, { estado: !row.estado });
    try {
      const resultado = await firstValueFrom(this.api.alternarEstado(row.colorid));
      if (typeof resultado?.estado === 'boolean') {
        this.patchRow(row.colorid, { estado: resultado.estado });
      }
    } catch {
      this.patchRow(row.colorid, { estado: row.estado });
    }
  ...
  private patchRow(id: number, patch: Partial<Color>): void {
    this.items.update((list) => list.map((it) => (it.colorid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<Color[]>([]);

  protected onSeleccion(filas: Color[]): void {
    this.seleccionadas.set(filas);
  }

  /**
   * Borrado en lote: UNA petición, no N. El endpoint responde 200 con el resultado por id
   * porque el lote es parcial por diseño —un color con productos asignados falla mientras el
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.colorid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) {
        const borrados = new Set(resultados.filter((res) => res.ok).map((res) => res.colorid));
        this.items.update((list) => list.filter((it) => !borrados.has(it.colorid)));
   ...
   */
  protected async onReorder(filas: Color[]): Promise<void> {
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(COLOR_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(COLOR_MAX_ABREVIATURA)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar color' : 'Nuevo color',
  );
  ...
  protected onEditar(row: Color): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: ColorInput = this.form.getRawValue();
    const enEdicion = this.editando();
    this.guardando.set(true);

    try {
      if (enEdicion) {
        const actualizado = await firstValueFrom(this.api.update(enEdicion.colorid, input));
        this.patchRow(enEdicion.colorid, actualizado);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: Color): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      await firstValueFrom(this.api.remove(row.colorid));
      this.items.update((list) => list.filter((it) => it.colorid !== row.colorid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.colorid !== row.colorid));
  ...
  protected onExportar(e: ExportChoice): void {
    ...
    if (e.formato === 'csv') {
      exportCsv('colores.csv', matriz);
    } else {
      exportXls('colores.xls', matriz);
    }
  }
}
