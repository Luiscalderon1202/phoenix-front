...
import type { Laboratorio, LaboratorioInput, LaboratorioListQuery, ResultadoLoteLaboratorio } from '@phoenix/catalogo/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { LaboratorioListPage } from './laboratorio-list-page';

function laboratorio(over: Partial<Laboratorio> = {}): Laboratorio {
  return {
    laboratorioid: 1,
    nombre: 'BAYER',
    abreviatura: 'BAY',
 ...
 */
class LaboratorioApiMock {
  rows: Laboratorio[] = [
    laboratorio(),
    laboratorio({ laboratorioid: 2, nombre: 'PFIZER', abreviatura: 'PFZ', count_productos: 3 }),
  ];

  /** Las queries que ha recibido `list()`, en orden. */
  readonly consultas: LaboratorioListQuery[] = [];
  readonly creados: LaboratorioInput[] = [];
  readonly actualizados: { id: number; input: LaboratorioInput }[] = [];
  readonly eliminados: number[] = [];
  readonly lotes: readonly number[][] = [];

  /** Resultado por id del lote; por defecto todo OK. */
  loteResultado: ResultadoLoteLaboratorio[] | null = null;
  ...
  list(query: LaboratorioListQuery) {
    ...
    const data = filtrados.slice(inicio, inicio + query.page_size);
    const total = this.totalDeclarado ?? filtrados.length;
  ...
  create(input: LaboratorioInput) {
    this.creados.push(input);
    if (this.fallar) return throwError(() => new Error('409'));
    const creado = laboratorio({ laboratorioid: 99, ...input, count_productos: 0 });
    ...
    this.rows = [creado, ...this.rows];
    return of(creado);
  }

  update(id: number, input: LaboratorioInput) {
    this.actualizados.push({ id, input });
    if (this.fallar) return throwError(() => new Error('409'));
    const previa = this.rows.find((row) => row.laboratorioid === id);
    const actualizado = laboratorio({ ...previa, laboratorioid: id, ...input });
    this.rows = this.rows.map((row) => (row.laboratorioid === id ? actualizado : row));
  ...
  remove(id: number) {
    ...
    if (this.fallar) return throwError(() => new Error('409'));
    this.rows = this.rows.filter((row) => row.laboratorioid !== id);
  ...
  removeLote(ids: readonly number[]) {
    ...
    const resultados = this.loteResultado ?? ids.map((laboratorioid) => ({ laboratorioid, ok: true }));
    const borrados = new Set(resultados.filter((r) => r.ok).map((r) => r.laboratorioid));
    this.rows = this.rows.filter((row) => !borrados.has(row.laboratorioid));
...
async function setup(api = new LaboratorioApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [LaboratorioListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: LaboratorioApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(LaboratorioListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): LaboratorioInput };
    columns: { key: string; header?: string; sortable?: boolean }[];
    rows(): Laboratorio[];
    modalAbierto(): boolean;
    tituloModal(): string;
    seleccionadas(): { laboratorioid: number }[];
...
function ultima(api: LaboratorioApiMock): LaboratorioListQuery {
...
function catalogo(n: number): Laboratorio[] {
  return Array.from({ length: n }, (_, i) =>
    laboratorio({ laboratorioid: i + 1, nombre: `MARCA ${String(i + 1).padStart(2, '0')}`, abreviatura: `M${i + 1}` }),
  );
}

describe('LaboratorioListPage', () => {
  ...
  it('pinta la página que devuelve el backend', async () => {
    ...
    expect(celdas(el, 0)).toEqual(['BAYER', 'PFIZER']);
  });
  ...
  it('el filtro por nombre VA AL SERVIDOR', async () => {
    // Es la corrección deliberada del legacy: `ajLaboratorio.php:19` llama `Leer($vNombre)`
    // contra `Leer($vInicio, $vFin, $vNombre)`, así que el texto cae en el offset y la llamada
    // real acaba siendo siempre `palaboratorio_leer(0, 0, '')`. Aquí el texto llega.
    const { fixture, api } = await setup();
    const c = comp(fixture);

    c.filters.patchValue({ q: '  pfizer  ' });
    c.onApply();
    await asentar(fixture);

    expect(ultima(api).q).toBe('pfizer'); // recortado
    expect(ultima(api).page).toBe(1); // el filtro cambia el universo: se vuelve a la 1
    expect(c.rows().map((row) => row.nombre)).toEqual(['PFIZER']);
  });
  ...
  it('el total del pie sale del `meta` del backend, no de contar las filas cargadas', async () => {
    // `meta.total` lo calcula `palaboratorio_count(q)`. Contar `rows()` daría 25 en una tabla de
  ...
  it('la tabla no tiene columna de estado ni de orden', async () => {
    // `catalogo.laboratorio` son tres columnas: laboratorioid, nombre y abreviatura. El legacy
    // aparenta lo contrario invocando `palaboratorio_cambiar_estado`, que no existe.
    const { fixture, el } = await setup();
  ...
  it('el alta manda SOLO los dos campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'ROCHE', abreviatura: 'RCH' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'ROCHE', abreviatura: 'RCH' }]);
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'BAYER', abreviatura: 'BAY' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue()).toEqual({ nombre: 'BAYER', abreviatura: 'BAY' });
  });
  ...
  it('tras crear se RECARGA del servidor en vez de añadir la fila en local', async () => {
    ...
    expect(api.consultas.length).toBeGreaterThan(antes);
    expect(c.rows().map((row) => row.nombre)).toEqual(['ZONA', 'BAYER', 'PFIZER']);
  ...
  it('editar precarga los dos campos y manda un PUT', async () => {
    ...
    c.onEditar(laboratorio({ laboratorioid: 2, nombre: 'PFIZER', abreviatura: 'PFZ' }) as never);
    expect(c.tituloModal()).toBe('Editar laboratorio');
    expect(c.form.getRawValue()).toEqual({ nombre: 'PFIZER', abreviatura: 'PFZ' });

    c.form.patchValue({ nombre: 'PFIZER EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados).toEqual([
      { id: 2, input: { nombre: 'PFIZER EDITADO', abreviatura: 'PFZ' } },
    ]);
    expect(api.creados).toEqual([]);
    expect(c.rows().map((row) => row.nombre)).toEqual(['BAYER', 'PFIZER EDITADO']);
  });
  ...
  it('eliminar pide confirmación antes de llamar al backend', async () => {
    ...
    c.onEliminar(laboratorio() as never);
    await asentar(fixture);
  ...
  it('tras eliminar se RECARGA del servidor', async () => {
    ...
    expect(api.consultas.length).toBeGreaterThan(antes);
    expect(c.rows().map((row) => row.laboratorioid)).toEqual([2]);
  ...
  it('el lote es parcial: conserva las que fallaron y avisa con el motivo del backend', async () => {
    const api = new LaboratorioApiMock();
    api.loteResultado = [
      { laboratorioid: 1, ok: true },
      { laboratorioid: 2, ok: false, codigo: 'laboratorio_has_relations', mensaje: 'Tiene productos registrados.' },
    ];
    const { fixture, notify } = await setup(api);
    const c = comp(fixture);

    c.onSeleccion([{ laboratorioid: 1 }, { laboratorioid: 2 }] as never);
  ...
  it('sin productos, la celda queda vacía', async () => {
    ...
    const api = new LaboratorioApiMock();
    api.rows = [laboratorio({ count_productos: 0 })];
  ...
  it('con un solo producto lo dice con palabras, sin badge', async () => {
    const api = new LaboratorioApiMock();
    api.rows = [laboratorio({ count_productos: 1 })];
    const { el } = await setup(api);
  ...
  it('con varios productos pinta el número en un badge', async () => {
    const api = new LaboratorioApiMock();
    api.rows = [laboratorio({ count_productos: 12 })];
    const { el } = await setup(api);
  ...
  it('sin historia previa, cerrar vuelve a /inicio', async () => {
    // ⚠ NO a /mantenimiento/tablas-basicas: Laboratorios no cuelga de ese hub, es una opción de
