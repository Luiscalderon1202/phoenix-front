...
import { LaboratorioApi } from '@phoenix/catalogo/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  LABORATORIO_MAX_ABREVIATURA,
  LABORATORIO_MAX_NOMBRE,
  type Laboratorio,
  type LaboratorioInput,
  type LaboratorioListQuery,
...
/**
 * Mantenimiento de Laboratorios (`catalogo.laboratorio`, menuweb 91, proceso `CAT-LABORATORIO`).
 *
 * A diferencia de la línea y la marca, el laboratorio cuelga DIRECTAMENTE del producto
 * (`catalogo.producto.laboratorioid`), no del master.
 *
 * ⚠ Es el gemelo exacto de Marcas, y por los mismos dos motivos:
 *
 * - **PAGINA y FILTRA EN SERVIDOR.** `page`, `page_size` y `q` entran en la query del
 *   `resource`, así que cambiar cualquiera de los tres re-dispara la petición; la grilla recibe
 *   `rows()` a pelo, sin trocear, porque la respuesta ya viene troceada. El `total` del pie sale
 *   de `meta`, no de contar filas en memoria.
 * - **Sin columna `#`, sin columna de estado y sin arrastre.** `catalogo.laboratorio` tiene tres
 *   columnas —`laboratorioid`, `nombre`, `abreviatura`— y no existe ni `estado` ni `orden`. El
 *   legacy aparenta lo contrario con código muerto en sus tres capas: `ajLaboratorio.php:57`
 *   invoca `catalogo.palaboratorio_cambiar_estado`, una función que NO EXISTE en la base y sobre
 *   una columna que tampoco, y su `change_order` llama a un método `Cambiar_Orden` ausente de
 *   `Acceso_clsLaboratorio`. No se migra nada de eso.
 *
 * ⚠⚠ EN PRODUCCIÓN, BUSCAR LABORATORIOS POR NOMBRE NO HACE NADA. `ajLaboratorio.php:19` llama
 * `$oL->Leer($vNombre)` contra una firma `Leer($vInicio=0, $vFin=0, $vNombre='')`: el texto cae
 * en `$vInicio`, `intval()` lo vuelve 0 y la llamada real es SIEMPRE
 * `palaboratorio_leer(0, 0, '')`. Es el mismo bug exacto de `ajMarca.php:17`. La función SQL sí
 * sabe paginar y filtrar —tiene su `palaboratorio_count` y todo, sin un solo llamador en el
 * repo—, simplemente nadie la llamaba bien. Esta pantalla la usa como fue diseñada: es una
 * CORRECCIÓN DELIBERADA del legacy, no una divergencia accidental.
 *
 * ⚠ En producción esta tabla tiene UNA sola fila, la centinela «NO DEFINIDO» (id 0), que el
 * listado oculta: hasta que alguien dé de alta el primer laboratorio, la pantalla sale vacía. No
 * es un fallo de carga.
 *
 * Sin reporte PDF ni menú Imprimir: este recurso no lo tiene.
 *
 * Y esta pantalla NO cuelga del hub de Tablas Básicas —es una opción de menú propia con su
 * propio permiso—, así que cerrar vuelve a `/inicio`.
 */
@Component({
  selector: 'erp-laboratorio-list-page',
  ...
  templateUrl: './laboratorio-list-page.html',
  styleUrl: './laboratorio-list-page.scss',
})
export class LaboratorioListPage {
  private readonly api = inject(LaboratorioApi);
   ...
   *
   * ⚠ NO se vuelve a `/mantenimiento/tablas-basicas`: Laboratorios no cuelga de ese hub, es una
  ...
  protected readonly maxAbreviatura = LABORATORIO_MAX_ABREVIATURA;
  protected readonly maxNombre = LABORATORIO_MAX_NOMBRE;
   ...
   */
  private readonly query = computed<LaboratorioListQuery>(() => ({
    page: this.page(),
    page_size: this.pageSize(),
    q: this.applied().q.trim(),
  }));

  private readonly laboratorios = resource({
    params: () => this.query(),
   ...
   */
  private readonly items = linkedSignal<Laboratorio[]>(() => this.laboratorios.value()?.data ?? []);
  protected readonly rows = this.items;

  protected readonly loading = this.laboratorios.isLoading;
  protected readonly error = computed(() => {
    const e = this.laboratorios.error();
    return e instanceof Error ? e.message : e ? 'No se pudo cargar el catálogo de laboratorios.' : null;
  });
   ...
   * de `palaboratorio_count(q)`, y de ahí se deriva el número de páginas.
   */
  protected readonly meta = computed<PageMeta>(() => {
    const page = this.page();
    const pageSize = this.pageSize();
    const total = this.laboratorios.value()?.meta.total ?? 0;
  ...
  protected readonly columns: GridColumn<Laboratorio>[] = [
   ...
   */
  private patchRow(laboratorioid: number, patch: Partial<Laboratorio>): void {
    this.items.update((list) => list.map((it) => (it.laboratorioid === laboratorioid ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<Laboratorio[]>([]);

  protected onSeleccion(filas: Laboratorio[]): void {
    this.seleccionadas.set(filas);
  }

  /**
   * Borrado en lote: UNA petición, no N. El endpoint responde 200 con el resultado por id
   * porque el lote es parcial por diseño —un laboratorio puede fallar por tener masters mientras
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.laboratorioid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) this.notify.success(`Se eliminaron ${eliminados} registro(s).`);
      if (fallos.length > 0) {
  ...
  protected readonly editando = signal<Laboratorio | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(LABORATORIO_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(LABORATORIO_MAX_ABREVIATURA)]],
  });

  protected readonly tituloModal = computed(() => (this.editando() ? 'Editar laboratorio' : 'Nuevo laboratorio'));
  ...
  protected onEditar(row: Laboratorio): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: LaboratorioInput = this.form.getRawValue();
    ...
    try {
      if (enEdicion) {
        ...
        const actualizado = await firstValueFrom(this.api.update(enEdicion.laboratorioid, input));
        this.patchRow(enEdicion.laboratorioid, actualizado);
        ...
        // ⚠ Aquí NO se hace `items.update([...list, creado])` como en las pantallas de
        // catálogo completo. Con paginación de servidor, añadir la fila en local descuadra el
        // `total` del pie —que viene de `palaboratorio_count`, no de contar filas— y deja la página
        // en curso con 26 registros de 25. Además el laboratorio nueva puede pertenecer a otra
   ...
   * tiene que aparecer.
   */
  protected async onEliminar(row: Laboratorio): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      await firstValueFrom(this.api.remove(row.laboratorioid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.laboratorioid !== row.laboratorioid));
  ...
  protected onExportar(e: ExportChoice): void {
    ...
    const matriz = [
      cabeceras,
      ...this.rows().map((m) => [m.laboratorioid, m.nombre, m.abreviatura, m.count_productos]),
    ];

    if (e.formato === 'csv') {
      exportCsv('laboratorios.csv', matriz);
    } else {
      exportXls('laboratorios.xls', matriz);
    }
  }
}
