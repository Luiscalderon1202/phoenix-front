...
import { GrupoListPage } from './grupo-list-page';
 ...
 *
 * ⚠ `list()` FILTRA POR NOMBRE (como `catalogo.pagrupo_leer`) pero NO por estado ni por página:
...
class GrupoApiMock {
  rows: Grupo[] = [
    grupo(),
    grupo({ grupoid: 2, nombre: 'SERVICIOS', abreviatura: 'SERV', orden: 2, cantidad_productos: 3 }),
  ];

  /** Los filtros que ha recibido `list()`, en orden. */
  readonly consultas: GrupoFiltros[] = [];
  readonly creados: GrupoInput[] = [];
  readonly actualizados: { id: number; input: GrupoInput }[] = [];
  ...
  loteResultado: ResultadoLoteGrupo[] | null = null;
  ...
  create(input: GrupoInput) {
    ...
    const creado = grupo({ grupoid: 99, ...input, orden: this.rows.length + 1 });
    this.rows = [...this.rows, creado];
    return of(creado);
  }

  update(id: number, input: GrupoInput) {
    this.actualizados.push({ id, input });
    if (this.fallar) return throwError(() => new Error('409'));
    const previo = this.rows.find((row) => row.grupoid === id);
    const actualizado = grupo({ ...previo, grupoid: id, ...input });
    this.rows = this.rows.map((row) => (row.grupoid === id ? actualizado : row));
  ...
  remove(id: number) {
    ...
    if (this.fallar) return throwError(() => new Error('409'));
    this.rows = this.rows.filter((row) => row.grupoid !== id);
  ...
  removeLote(ids: readonly number[]) {
    ...
    const resultados = this.loteResultado ?? ids.map((grupoid) => ({ grupoid, ok: true }));
    const borrados = new Set(resultados.filter((r) => r.ok).map((r) => r.grupoid));
    this.rows = this.rows.filter((row) => !borrados.has(row.grupoid));
    return of(resultados);
  }

  alternarEstado(grupoid: number) {
    this.alternados.push(grupoid);
    if (this.fallar) return throwError(() => new Error('500'));
    return of({ grupoid, estado: this.estadoResultante });
  }
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): GrupoInput };
    columns: { key: string; header?: string; sortable?: boolean }[];
    rows(): Grupo[];
    rowsPagina(): Grupo[];
    modalAbierto(): boolean;
    tituloModal(): string;
    seleccionadas(): { grupoid: number }[];
    filtrando(): boolean;
...
function ultima(api: GrupoApiMock): GrupoFiltros {
...
function catalogo(n: number): Grupo[] {
  return Array.from({ length: n }, (_, i) =>
    grupo({
      grupoid: i + 1,
      nombre: `GRUPO ${String(i + 1).padStart(2, '0')}`,
      abreviatura: `G${i + 1}`,
      orden: i + 1,
    }),
  );
...
describe('GrupoListPage', () => {
  ...
  it('pinta el catálogo con la columna de orden y la de estado', async () => {
    ...
    expect(celdas(el, 1)).toEqual(['PRODUCTOS', 'SERVICIOS']);
  });
  ...
  it('arranca mostrando activos e inactivos, sin mandarle el estado al backend', async () => {
    // `catalogo.pagrupo_leer` no tiene parámetro de estado: devuelve todo. Por eso el selector
    // arranca en «Todos» y el filtro se resuelve en el cliente.
    const api = new GrupoApiMock();
    api.rows = [grupo(), grupo({ grupoid: 2, nombre: 'APAGADO', estado: false, orden: 2 })];
    const { fixture } = await setup(api);

    expect(ultima(api)).toEqual({ q: '' });
    expect(comp(fixture).rows().map((row) => row.nombre)).toEqual(['PRODUCTOS', 'APAGADO']);
  });
  ...
  it('el filtro por nombre VA AL SERVIDOR', async () => {
    ...
    c.filters.patchValue({ q: 'servicios' });
    c.onApply();
    await asentar(fixture);

    expect(ultima(api).q).toBe('servicios');
    expect(c.rows().map((row) => row.nombre)).toEqual(['SERVICIOS']);
  });
  ...
  it('el filtro de estado se resuelve EN CLIENTE, sin volver a consultar', async () => {
    ...
    c.onApply();
    await asentar(fixture);
    expect(c.rows().map((row) => row.nombre)).toEqual(['PRODUCTOS']);
  });

  it('la paginación es de CLIENTE: trocea lo que ya trajo', async () => {
    // El endpoint devuelve el catálogo entero y sin `meta`: no existe `pagrupo_count`.
    const api = new GrupoApiMock();
    ...
    expect(c.rowsPagina()[0].nombre).toBe('GRUPO 26');
  });
  ...
  it('alterna el estado y se queda con el valor que devuelve el backend', async () => {
    ...
    c.onToggleEstado(grupo() as never);
    await asentar(fixture);
  ...
  it('al reordenar manda la lista completa desde la posición 1 y renumera en local', async () => {
    // ⚠ Es lo que exige la función del legacy... y lo que ésta NO sabía hacer:
    // `catalogo.pagrupo_cambiar_orden(myarr)` no recibe `vstart` y numera siempre desde 1. La
    // migración 0009 aporta la versión con `vstart`, y la pantalla le manda 1 porque solo deja
    // arrastrar con el catálogo entero a la vista.
    const { fixture, api } = await setup();
    const c = comp(fixture);
    ...
    expect(c.rows().map((row) => [row.grupoid, row.orden])).toEqual([
      [2, 1],
      [1, 2],
    ]);
  });
  ...
  it('revierte el orden si el backend falla', async () => {
    ...
    const antes = c.rows().map((row) => row.grupoid);
    c.onReorder([...c.rows()].reverse() as never);
    await asentar(fixture);

    expect(c.rows().map((row) => row.grupoid)).toEqual(antes);
  });
  ...
  it('el arrastre solo se enciende con el catálogo entero a la vista', async () => {
    ...
    c.filters.patchValue({ estado: 'todas', q: 'servicios' });
    c.onApply();
    await asentar(fixture);
    expect(c.filtrando()).toBe(true);
  });
  ...
  it('el alta manda SOLO los dos campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'PROMOCIONES', abreviatura: 'PROM' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'PROMOCIONES', abreviatura: 'PROM' }]);
  ...
  it('tras crear añade la fila al final sin volver al servidor', async () => {
    // El backend coloca el alta al final del orden (`pagrupo_ultimo_orden()+1`), así que la
    ...
    expect(c.rows().map((row) => row.nombre)).toEqual(['PRODUCTOS', 'SERVICIOS', 'PROMOCIONES']);
  });
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'PRODUCTOS', abreviatura: 'PROD' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue()).toEqual({ nombre: 'PRODUCTOS', abreviatura: 'PROD' });
  });
  ...
  it('editar precarga los dos campos y manda un PUT', async () => {
    ...
    c.onEditar(grupo({ grupoid: 2, nombre: 'SERVICIOS', abreviatura: 'SERV' }) as never);
    expect(c.tituloModal()).toBe('Editar grupo');
    expect(c.form.getRawValue()).toEqual({ nombre: 'SERVICIOS', abreviatura: 'SERV' });

    c.form.patchValue({ nombre: 'SERVICIOS TECNICOS' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados).toEqual([
      { id: 2, input: { nombre: 'SERVICIOS TECNICOS', abreviatura: 'SERV' } },
    ]);
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia ni el estado ni el orden del registro', async () => {
    ...
    const api = new GrupoApiMock();
    api.rows = [grupo({ grupoid: 1, estado: false, orden: 7 })];
    const { fixture } = await setup(api);
    const c = comp(fixture);
  ...
  it('elimina y quita la fila de la lista', async () => {
    ...
    expect(c.rows().map((row) => row.grupoid)).toEqual([2]);
    expect(notify.exitos.length).toBe(1);
  });
  ...
  it('el lote es parcial: conserva los que fallaron y avisa con el motivo del backend', async () => {
    ...
    api.loteResultado = [
      { grupoid: 1, ok: true },
      { grupoid: 2, ok: false, codigo: 'grupo_has_relations', mensaje: 'Tiene productos registrados.' },
    ];
    const { fixture, notify } = await setup(api);
    const c = comp(fixture);

    c.onSeleccion([{ grupoid: 1 }, { grupoid: 2 }] as never);
  ...
  it('sin productos, la celda queda en gris', async () => {
    ...
    const api = new GrupoApiMock();
    api.rows = [grupo({ cantidad_productos: 0 })];
  ...
  it('con un solo producto lo dice con palabras, sin badge', async () => {
    const api = new GrupoApiMock();
    api.rows = [grupo({ cantidad_productos: 1 })];
    const { el } = await setup(api);
  ...
  it('con varios productos pinta el número en un badge', async () => {
    const api = new GrupoApiMock();
    api.rows = [grupo({ cantidad_productos: 12 })];
    const { el } = await setup(api);
  ...
  it('sin historia previa, cerrar vuelve a /inicio', async () => {
    // ⚠ NO a /mantenimiento/tablas-basicas: Grupos no cuelga de ese hub.
    const { fixture } = await setup();
