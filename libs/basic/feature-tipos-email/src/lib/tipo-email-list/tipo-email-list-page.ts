...
/**
 * Mantenimiento de Tipos de Email (`basic.tipoemail`).
 *
 ...
 */
@Component({
  selector: 'erp-tipo-email-list-page',
  ...
  templateUrl: './tipo-email-list-page.html',
  styleUrl: './tipo-email-list-page.scss',
})
export class TipoEmailListPage {
  private readonly api = inject(TipoEmailApi);
  ...
  protected readonly maxNombre = TIPO_EMAIL_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<TipoEmail[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de email.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoEmail[]>(() => {
  ...
  protected readonly rowsPagina = computed<TipoEmail[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoEmail>[] = [
   ...
   */
  protected async onToggleEstado(row: TipoEmail): Promise<void> {
  ...
  private patchRow(id: number, patch: Partial<TipoEmail>): void {
    this.items.update((list) => list.map((it) => (it.tipoid === id ? { ...it, ...patch } : it)));
  }

  /**
   * `pordefecto` es EXCLUYENTE en la base: el stored procedure remata cada escritura con
   * `set pordefecto=false where tipoid != <el guardado>`. El backend solo devuelve la fila
   * escrita, así que la lista local se queda con dos marcadas si no se replica aquí.
   */
  private desmarcarOtrosPorDefecto(id: number): void {
    this.items.update((list) =>
      list.map((it) => (it.tipoid === id ? it : { ...it, pordefecto: false })),
    );
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoEmail[]>([]);

  protected onSeleccion(filas: TipoEmail[]): void {
    this.seleccionadas.set(filas);
  }

  /**
   * Borrado en lote contra `POST /tipos-email/lote/eliminar`: UNA petición, no N. El endpoint
   * responde 200 con el resultado por id porque el lote es parcial por diseño —uno puede
   * estar en uso mientras el resto sí se borra—, así que el saldo se lee de las filas.
   */
   ...
   */
  protected async onReorder(filas: TipoEmail[]): Promise<void> {
  ...
  protected readonly editando = signal<TipoEmail | null>(null);
  protected readonly guardando = signal(false);

  /**
   * Los tres campos que acepta el backend. Ni `orden` ni `estado`: cada uno tiene su propia
   * acción en la grilla —arrastrar la fila y pulsar su etiqueta de estado— y ésa es su única
   * fuente de verdad. El SP del legacy sí recibe el estado, pero el backend le devuelve el
   * que la fila ya tenía, así que guardar aquí no lo altera.
   */
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_EMAIL_MAX_NOMBRE)]],
    requerido: [false],
    pordefecto: [false],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de email' : 'Nuevo tipo de email',
  );
  ...
  protected onEditar(row: TipoEmail): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoEmailInput = this.form.getRawValue();
    ...
    try {
      if (enEdicion) {
        ...
        if (actualizado.pordefecto) this.desmarcarOtrosPorDefecto(actualizado.tipoid);
        ...
        if (creado.pordefecto) this.desmarcarOtrosPorDefecto(creado.tipoid);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoEmail): Promise<void> {
    const ok = await this.confirm.ask({
      title: 'Eliminar tipo de email',
    ...
    try {
      ...
      // 409 si hay personas con un email de este tipo: ya lo notificó el interceptor. Se
  ...
  protected onExportar(e: ExportChoice): void {
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-email.csv', matriz);
    } else {
      exportXls('tipos-email.xls', matriz);
    }
  }
}
