...
import { TipoVentaApi } from '@phoenix/basic/data-access';
import type { TipoVenta, TipoVentaInput } from '@phoenix/basic/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { TipoVentaListPage } from './tipo-venta-list-page';

function fila(over: Partial<TipoVenta> = {}): TipoVenta {
  return {
    tipoventaid: 1,
    nombre: 'CONTADO',
    abreviatura: 'CON',
    codigo_contable: '70111',
    anticipo: false,
    orden: 1,
    estado: true,
    ...over,
...
class TipoVentaApiMock {
  rows: TipoVenta[] = [fila(), fila({ tipoventaid: 2, nombre: 'CREDITO', abreviatura: 'CRE', codigo_contable: '70112', anticipo: true, orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: TipoVentaInput[] = [];
  readonly actualizados: { id: number; input: TipoVentaInput }[] = [];
  readonly reordenes: { ids: readonly number[] }[] = [];
  readonly lotes: readonly number[][] = [];
  /** Resultado por id del lote; por defecto todo OK. */
  loteResultado: { tipoventaid: number; ok: boolean; mensaje?: string }[] | null = null;
  ...
  create(input: TipoVentaInput) {
    this.creados.push(input);
    if (this.fallar) return throwError(() => new Error('409'));
    return of(fila({ tipoventaid: 99, ...input, orden: this.rows.length + 1 }));
  }

  update(id: number, input: TipoVentaInput) {
    this.actualizados.push({ id, input });
    if (this.fallar) return throwError(() => new Error('409'));
    // El PUT no lleva estado ni orden, así que la respuesta conserva los previos.
    const previo = this.rows.find((row) => row.tipoventaid === id);
    return of(fila({ ...previo, tipoventaid: id, ...input }));
  }
  ...
  alternarEstado(tipoventaid: number) {
    this.alternados.push(tipoventaid);
    if (this.fallar) return throwError(() => new Error('500'));
    return of({ tipoventaid, estado: this.estadoResultante });
  }

  reordenar(ids: readonly number[]) {
    this.reordenes.push({ ids: [...ids] });
    if (this.fallar) return throwError(() => new Error('500'));
    return of(undefined as unknown as void);
  }
  ...
  removeLote(ids: readonly number[]) {
    ...
    if (this.fallar) return throwError(() => new Error('500'));
    return of(this.loteResultado ?? ids.map((tipoventaid) => ({ tipoventaid, ok: true })));
  }
}
...
async function setup(api = new TipoVentaApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [TipoVentaListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: TipoVentaApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(TipoVentaListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): TipoVentaInput };
    rows(): TipoVenta[];
    filtrando(): boolean;
    modalAbierto(): boolean;
    tituloModal(): string;
    seleccionadas(): { tipoventaid: number }[];
    onExportar(e: { formato: string; detalle: boolean }): void;
    onCerrar(): void;
    onPageSize(n: number): void;
    onPage(n: number): void;
    rowsPagina(): TipoVenta[];
...
describe('TipoVentaListPage', () => {
  ...
  it('filtra en cliente por texto: no hay endpoint al que mandar el filtro', async () => {
    ...
    c.filters.patchValue({ q: 'credit' });
    c.onApply();
    fixture.detectChanges();

    expect(c.rows().map((row) => row.tipoventaid)).toEqual([2]);
  });

  it('filtra por estado', async () => {
    const api = new TipoVentaApiMock();
    api.rows = [fila(), fila({ tipoventaid: 2, estado: false })];
  ...
  it('al reordenar manda la lista completa y SIN offset, y renumera en local', async () => {
    // Este recurso no tiene `desde`: su stored procedure no recibe `vstart` y numera
    // desde 1, así que mandar un tramo suelto lo colaría delante del resto.
    ...
    expect(api.reordenes).toEqual([{ ids: [2, 1] }]);
    expect(c.rows().map((row) => [row.tipoventaid, row.orden])).toEqual([
      [2, 1],
      [1, 2],
    ]);
  });
  ...
  it('revierte el orden si el backend falla', async () => {
    ...
    expect(c.rows().map((row) => row.tipoventaid)).toEqual([1, 2]);
  });
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'ANTICIPO', abreviatura: 'ANT', codigo_contable: '70113', anticipo: true });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'ANTICIPO', abreviatura: 'ANT', codigo_contable: '70113', anticipo: true }]);
  ...
  it('el formulario no ofrece ni orden ni estado', async () => {
    ...
    expect(c.form.getRawValue()).toEqual({ nombre: '', abreviatura: '', codigo_contable: '', anticipo: false });
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'CONTADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('CONTADO');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar tipo de venta');
    expect(c.form.getRawValue()).toEqual({ nombre: 'CONTADO', abreviatura: 'CON', codigo_contable: '70111', anticipo: false });

    c.form.patchValue({ nombre: 'CONTADO EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('CONTADO EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new TipoVentaApiMock();
    api.rows = [fila(), fila({ tipoventaid: 2, nombre: 'CREDITO', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'CREDITO EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados[0].input).not.toHaveProperty('estado');
    expect(c.rows().map((row) => [row.tipoventaid, row.estado])).toEqual([
      [1, true],
      [2, false],
    ]);
  });
  ...
  it('el borrado en lote va en UNA petición y quita las filas', async () => {
    ...
    c.onSeleccion([{ tipoventaid: 1 }, { tipoventaid: 2 }] as never);
  ...
  it('el lote es parcial: conserva las filas que fallaron', async () => {
    const api = new TipoVentaApiMock();
    api.loteResultado = [
      { tipoventaid: 1, ok: true },
      { tipoventaid: 2, ok: false, mensaje: 'Tiene registros relacionados.' },
    ];
  ...
  it('el borrado en lote pide confirmación', async () => {
    ...
    c.onSeleccion([{ tipoventaid: 1 }] as never);
    c.onEliminarSeleccionadas();
    await asentar(fixture);
  ...
  it('pagina en cliente: el endpoint devuelve el catálogo entero', async () => {
    ...
    api.rows = Array.from({ length: 30 }, (_, i) =>
      fila({ tipoventaid: i + 1, nombre: `FILA ${i + 1}`, orden: i + 1 }),
    );
  ...
  it('pinta «anticipo» en palabras', async () => {
    const { el } = await setup();
    expect(celdas(el, 4)).toEqual(['No', 'Sí']);
  });

  it('«anticipo» viaja en el cuerpo y la edición puede apagarlo', async () => {
    // Es un dato del tipo de venta, no un interruptor con ruta propia como el estado.
    const { fixture, api } = await setup();
    const c = comp(fixture);

    c.onEditar(fila({ tipoventaid: 2, nombre: 'CREDITO', anticipo: true }) as never);
    c.form.patchValue({ anticipo: false });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados[0].input.anticipo).toBe(false);
  });
});
