...
import {
  TIPO_VENTA_MAX_ABREVIATURA,
  TIPO_VENTA_MAX_CODIGO_CONTABLE,
  TIPO_VENTA_MAX_NOMBRE,
  type TipoVenta,
  type TipoVentaInput,
 ...
 */
@Component({
  selector: 'erp-tipo-venta-list-page',
  ...
  templateUrl: './tipo-venta-list-page.html',
  styleUrl: './tipo-venta-list-page.scss',
})
export class TipoVentaListPage {
  private readonly api = inject(TipoVentaApi);
  ...
  protected readonly maxAbreviatura = TIPO_VENTA_MAX_ABREVIATURA;
  protected readonly maxCodigoContable = TIPO_VENTA_MAX_CODIGO_CONTABLE;
  protected readonly maxNombre = TIPO_VENTA_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<TipoVenta[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de venta.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoVenta[]>(() => {
  ...
  protected readonly rowsPagina = computed<TipoVenta[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoVenta>[] = [
    { key: 'orden', header: '#', width: '60px', align: 'right', mono: true },
    { key: 'nombre', header: 'Nombre' },
    { key: 'abreviatura', header: 'Abreviatura', width: '150px' },
    { key: 'codigo_contable', header: 'C. contable', width: '140px' },
    { key: 'anticipo', header: 'Anticipo', width: '120px' },
   ...
   */
  protected async onToggleEstado(row: TipoVenta): Promise<void> {
    this.patchRow(row.tipoventaid, { estado: !row.estado });
    try {
      const resultado = await firstValueFrom(this.api.alternarEstado(row.tipoventaid));
      if (typeof resultado?.estado === 'boolean') {
        this.patchRow(row.tipoventaid, { estado: resultado.estado });
      }
    } catch {
      this.patchRow(row.tipoventaid, { estado: row.estado });
    }
  ...
  private patchRow(id: number, patch: Partial<TipoVenta>): void {
    this.items.update((list) => list.map((it) => (it.tipoventaid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoVenta[]>([]);

  protected onSeleccion(filas: TipoVenta[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.tipoventaid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) {
        const borrados = new Set(resultados.filter((res) => res.ok).map((res) => res.tipoventaid));
        this.items.update((list) => list.filter((it) => !borrados.has(it.tipoventaid)));
   ...
   *
   * Sin `desde`: el stored procedure de este recurso no recibe offset y numera desde 1,
   * así que el endpoint espera la lista COMPLETA. Por eso se manda entera y no el tramo.
   *
   ...
   */
  protected async onReorder(filas: TipoVenta[]): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.reordenar(filas.map((f) => f.tipoventaid)));
    } catch {
  ...
  protected readonly editando = signal<TipoVenta | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_VENTA_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(TIPO_VENTA_MAX_ABREVIATURA)]],
    codigo_contable: ['', [Validators.maxLength(TIPO_VENTA_MAX_CODIGO_CONTABLE)]],
    anticipo: [false],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de venta' : 'Nuevo tipo de venta',
  );

  protected onNuevo(): void {
    this.editando.set(null);
    this.form.reset({ nombre: '', abreviatura: '', codigo_contable: '', anticipo: false });
    this.modalAbierto.set(true);
  }

  protected onEditar(row: TipoVenta): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoVentaInput = this.form.getRawValue();
    const enEdicion = this.editando();
    this.guardando.set(true);

    try {
      if (enEdicion) {
        const actualizado = await firstValueFrom(this.api.update(enEdicion.tipoventaid, input));
        this.patchRow(enEdicion.tipoventaid, actualizado);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoVenta): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      await firstValueFrom(this.api.remove(row.tipoventaid));
      this.items.update((list) => list.filter((it) => it.tipoventaid !== row.tipoventaid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.tipoventaid !== row.tipoventaid));
  ...
  protected onExportar(e: ExportChoice): void {
    const cabeceras = ['Orden', 'ID', 'Nombre', 'Abreviatura', 'C. contable', 'Anticipo', 'Estado'];
    const matriz = [
      cabeceras,
      ...this.rows().map((t) => [
        t.orden,
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-venta.csv', matriz);
    } else {
      exportXls('tipos-venta.xls', matriz);
    }
  }
}
