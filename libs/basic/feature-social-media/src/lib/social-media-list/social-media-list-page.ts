...
import {
  TIPO_SOCIAL_MEDIA_MAX_ICONO,
  TIPO_SOCIAL_MEDIA_MAX_NOMBRE,
  TIPO_SOCIAL_MEDIA_MAX_URL,
  type TipoSocialMedia,
  type TipoSocialMediaInput,
...
/**
 * Mantenimiento de Social media (`basic.tiposocialmedia`).
 *
 * Misma forma que Tipos de empresa —CRUD en modal, toggle de estado, reordenamiento
 * arrastrando y borrado en lote— con una diferencia: NO hay reporte PDF, porque el backend
 * no expone ninguno para este recurso. La cabecera solo lleva el menú Exportar (Excel/CSV),
 * que se genera en el navegador.
 *
 * El `estado` y el `orden` no están en el formulario aunque el stored procedure del legacy
 * acepte el primero: cada uno se cambia desde el listado —la etiqueta de estado y el
 * arrastre— y ésa es su única vía.
 *
 * El catálogo llega COMPLETO en una sola petición (la función del legacy no pagina y son
 * un puñado de filas). Por eso el filtro de texto, el de estado y la paginación se
 * resuelven en CLIENTE: no hay endpoint al que mandarlos.
 */
@Component({
  selector: 'erp-social-media-list-page',
  ...
  templateUrl: './social-media-list-page.html',
  styleUrl: './social-media-list-page.scss',
})
export class SocialMediaListPage {
  private readonly api = inject(TipoSocialMediaApi);
  ...
  protected readonly maxNombre = TIPO_SOCIAL_MEDIA_MAX_NOMBRE;
  protected readonly maxUrl = TIPO_SOCIAL_MEDIA_MAX_URL;
  protected readonly maxIcono = TIPO_SOCIAL_MEDIA_MAX_ICONO;
   ...
   */
  private readonly items = linkedSignal<TipoSocialMedia[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de redes sociales.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoSocialMedia[]>(() => {
    ...
    return this.items().filter((t) => {
      ...
      return (
        ...
        t.url.toLocaleLowerCase('es').includes(texto) ||
        t.icono.toLocaleLowerCase('es').includes(texto)
      );
    });
  });
  ...
  protected readonly rowsPagina = computed<TipoSocialMedia[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoSocialMedia>[] = [
    { key: 'orden', header: '#', width: '60px', align: 'right', mono: true },
    { key: 'nombre', header: 'Nombre', width: '200px' },
    { key: 'url', header: 'URL' },
    { key: 'icono', header: 'Icono', width: '180px' },
   ...
   */
  protected async onToggleEstado(row: TipoSocialMedia): Promise<void> {
  ...
  private patchRow(id: number, patch: Partial<TipoSocialMedia>): void {
    this.items.update((list) => list.map((it) => (it.tipoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoSocialMedia[]>([]);

  protected onSeleccion(filas: TipoSocialMedia[]): void {
    this.seleccionadas.set(filas);
  }

  /**
   * Borrado en lote contra `POST /tipos-social-media/lote/eliminar`: UNA petición, no N. El
   * endpoint responde 200 con el resultado por id porque el lote es parcial por diseño —uno
   * puede estar declarado por alguna persona mientras el resto sí se borra—, así que el saldo
   * se lee de las filas.
   */
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    try {
      ...
      if (fallos.length > 0) {
        // Los motivos se repiten ("tiene personas relacionadas"), así que se deduplican en
   ...
   */
  protected async onReorder(filas: TipoSocialMedia[]): Promise<void> {
  ...
  protected readonly editando = signal<TipoSocialMedia | null>(null);
  protected readonly guardando = signal(false);

  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_SOCIAL_MEDIA_MAX_NOMBRE)]],
    url: ['', [Validators.maxLength(TIPO_SOCIAL_MEDIA_MAX_URL)]],
    icono: ['', [Validators.maxLength(TIPO_SOCIAL_MEDIA_MAX_ICONO)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar red social' : 'Nueva red social',
  );

  protected onNuevo(): void {
    this.editando.set(null);
    this.form.reset({ nombre: '', url: '', icono: '' });
    this.modalAbierto.set(true);
  }

  protected onEditar(row: TipoSocialMedia): void {
    ...
    this.form.reset({ nombre: row.nombre, url: row.url, icono: row.icono });
    this.modalAbierto.set(true);
  }
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoSocialMediaInput = this.form.getRawValue();
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoSocialMedia): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      ...
      // 409 si alguna persona la tiene declarada: ya lo notificó el interceptor. Se recarga
   ...
   *
   * No hay menú Imprimir —este recurso no tiene reporte PDF— pero el navegador sí imprime
   * con Ctrl+P, y ahí el `erp-print-header` sustituye a la cabecera y al panel de filtros.
   */
   ...
   * navegador a partir de las filas ya cargadas, sin pedir nada al backend. Es el único menú
   * de salida de esta pantalla: no hay reporte PDF para este recurso.
   */
  protected onExportar(e: ExportChoice): void {
    const cabeceras = ['Orden', 'ID', 'Nombre', 'URL', 'Icono', 'Estado'];
    const matriz = [
      cabeceras,
      ...this.rows().map((t) => [
        t.orden,
    ...
    if (e.formato === 'csv') {
      exportCsv('social-media.csv', matriz);
    } else {
      exportXls('social-media.xls', matriz);
    }
  }
}
