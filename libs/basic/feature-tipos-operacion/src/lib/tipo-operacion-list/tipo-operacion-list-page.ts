...
import { TipoOperacionApi } from '@phoenix/basic/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  TIPO_OPERACION_MAX_ABREVIATURA,
  TIPO_OPERACION_MAX_CODIGO_CONTABLE,
  TIPO_OPERACION_MAX_NOMBRE,
  type TipoOperacion,
  type TipoOperacionInput,
...
/**
 * Mantenimiento de Tipos de operación.
 *
 ...
 */
@Component({
  selector: 'erp-tipo-operacion-list-page',
  ...
  templateUrl: './tipo-operacion-list-page.html',
  styleUrl: './tipo-operacion-list-page.scss',
})
export class TipoOperacionListPage {
  private readonly api = inject(TipoOperacionApi);
  ...
  protected readonly maxAbreviatura = TIPO_OPERACION_MAX_ABREVIATURA;
  protected readonly maxCodigoContable = TIPO_OPERACION_MAX_CODIGO_CONTABLE;
  protected readonly maxNombre = TIPO_OPERACION_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<TipoOperacion[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de operación.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoOperacion[]>(() => {
  ...
  protected readonly rowsPagina = computed<TipoOperacion[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoOperacion>[] = [
   ...
   */
  protected async onToggleEstado(row: TipoOperacion): Promise<void> {
    this.patchRow(row.tipooperacionid, { estado: !row.estado });
    try {
      const resultado = await firstValueFrom(this.api.alternarEstado(row.tipooperacionid));
      if (typeof resultado?.estado === 'boolean') {
        this.patchRow(row.tipooperacionid, { estado: resultado.estado });
      }
    } catch {
      this.patchRow(row.tipooperacionid, { estado: row.estado });
    }
  ...
  private patchRow(id: number, patch: Partial<TipoOperacion>): void {
    this.items.update((list) => list.map((it) => (it.tipooperacionid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoOperacion[]>([]);

  protected onSeleccion(filas: TipoOperacion[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar tipos de operación',
    ...
    try {
      const resultados = await firstValueFrom(this.api.removeLote(filas.map((f) => f.tipooperacionid)));
      const fallos = resultados.filter((res) => !res.ok);
      const eliminados = resultados.length - fallos.length;

      if (eliminados > 0) {
        const borrados = new Set(resultados.filter((res) => res.ok).map((res) => res.tipooperacionid));
        this.items.update((list) => list.filter((it) => !borrados.has(it.tipooperacionid)));
   ...
   */
  protected async onReorder(filas: TipoOperacion[]): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.reordenar(filas.map((f) => f.tipooperacionid), 1));
    } catch {
  ...
  protected readonly editando = signal<TipoOperacion | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_OPERACION_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(TIPO_OPERACION_MAX_ABREVIATURA)]],
    codigo_contable: ['', [Validators.maxLength(TIPO_OPERACION_MAX_CODIGO_CONTABLE)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de operación' : 'Nuevo tipo de operación',
  );
  ...
  protected onEditar(row: TipoOperacion): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoOperacionInput = this.form.getRawValue();
    const enEdicion = this.editando();
    this.guardando.set(true);

    try {
      if (enEdicion) {
        const actualizado = await firstValueFrom(this.api.update(enEdicion.tipooperacionid, input));
        this.patchRow(enEdicion.tipooperacionid, actualizado);
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoOperacion): Promise<void> {
    const ok = await this.confirm.ask({
    ...
    try {
      await firstValueFrom(this.api.remove(row.tipooperacionid));
      this.items.update((list) => list.filter((it) => it.tipooperacionid !== row.tipooperacionid));
      this.seleccionadas.update((sel) => sel.filter((s) => s.tipooperacionid !== row.tipooperacionid));
  ...
  protected onExportar(e: ExportChoice): void {
    const cabeceras = ['Orden', 'ID', 'Nombre', 'Abreviatura', 'C. contable', 'Estado'];
    const matriz = [
      cabeceras,
      ...this.rows().map((t) => [
        t.orden,
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-operacion.csv', matriz);
    } else {
      exportXls('tipos-operacion.xls', matriz);
    }
  }
}
