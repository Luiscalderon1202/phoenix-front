...
import type { TipoOperacion, TipoOperacionInput } from '@phoenix/basic/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { TipoOperacionListPage } from './tipo-operacion-list-page';

function fila(over: Partial<TipoOperacion> = {}): TipoOperacion {
  return {
    tipooperacionid: 1,
...
class TipoOperacionApiMock {
  rows: TipoOperacion[] = [fila(), fila({ tipooperacionid: 2, nombre: 'EXPORTACION', abreviatura: 'EXP', codigo_contable: '70112', orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: TipoOperacionInput[] = [];
  readonly actualizados: { id: number; input: TipoOperacionInput }[] = [];
  ...
  loteResultado: { tipooperacionid: number; ok: boolean; mensaje?: string }[] | null = null;
  ...
  create(input: TipoOperacionInput) {
    this.creados.push(input);
    if (this.fallar) return throwError(() => new Error('409'));
    return of(fila({ tipooperacionid: 99, ...input, orden: this.rows.length + 1 }));
  }

  update(id: number, input: TipoOperacionInput) {
    this.actualizados.push({ id, input });
    if (this.fallar) return throwError(() => new Error('409'));
    // El PUT no lleva estado ni orden, así que la respuesta conserva los previos.
    const previo = this.rows.find((row) => row.tipooperacionid === id);
    return of(fila({ ...previo, tipooperacionid: id, ...input }));
  }
  ...
  alternarEstado(tipooperacionid: number) {
    this.alternados.push(tipooperacionid);
    if (this.fallar) return throwError(() => new Error('500'));
    return of({ tipooperacionid, estado: this.estadoResultante });
  }
  ...
  removeLote(ids: readonly number[]) {
    ...
    if (this.fallar) return throwError(() => new Error('500'));
    return of(this.loteResultado ?? ids.map((tipooperacionid) => ({ tipooperacionid, ok: true })));
  }
}
...
async function setup(api = new TipoOperacionApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [TipoOperacionListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: TipoOperacionApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(TipoOperacionListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): TipoOperacionInput };
    rows(): TipoOperacion[];
    filtrando(): boolean;
    modalAbierto(): boolean;
    tituloModal(): string;
    seleccionadas(): { tipooperacionid: number }[];
    onExportar(e: { formato: string; detalle: boolean }): void;
    onCerrar(): void;
    onPageSize(n: number): void;
    onPage(n: number): void;
    rowsPagina(): TipoOperacion[];
...
describe('TipoOperacionListPage', () => {
  ...
  it('filtra en cliente por texto: no hay endpoint al que mandar el filtro', async () => {
    ...
    c.filters.patchValue({ q: 'export' });
    c.onApply();
    fixture.detectChanges();

    expect(c.rows().map((row) => row.tipooperacionid)).toEqual([2]);
  });

  it('filtra por estado', async () => {
    const api = new TipoOperacionApiMock();
    api.rows = [fila(), fila({ tipooperacionid: 2, estado: false })];
  ...
  it('al reordenar manda la lista completa desde la posición 1 y renumera en local', async () => {
    ...
    expect(c.rows().map((row) => [row.tipooperacionid, row.orden])).toEqual([
      [2, 1],
      [1, 2],
    ]);
  });
  ...
  it('revierte el orden si el backend falla', async () => {
    ...
    expect(c.rows().map((row) => row.tipooperacionid)).toEqual([1, 2]);
  });
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'GRATUITA', abreviatura: 'GRA', codigo_contable: '70113' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'GRATUITA', abreviatura: 'GRA', codigo_contable: '70113' }]);
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'VENTA INTERNA' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('VENTA INTERNA');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar tipo de operación');
    expect(c.form.getRawValue()).toEqual({ nombre: 'VENTA INTERNA', abreviatura: 'VI', codigo_contable: '70111' });

    c.form.patchValue({ nombre: 'VENTA INTERNA EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('VENTA INTERNA EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new TipoOperacionApiMock();
    api.rows = [fila(), fila({ tipooperacionid: 2, nombre: 'EXPORTACION', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'EXPORTACION EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados[0].input).not.toHaveProperty('estado');
    expect(c.rows().map((row) => [row.tipooperacionid, row.estado])).toEqual([
      [1, true],
      [2, false],
    ]);
  });
  ...
  it('el borrado en lote va en UNA petición y quita las filas', async () => {
    ...
    c.onSeleccion([{ tipooperacionid: 1 }, { tipooperacionid: 2 }] as never);
  ...
  it('el lote es parcial: conserva las filas que fallaron', async () => {
    const api = new TipoOperacionApiMock();
    api.loteResultado = [
      { tipooperacionid: 1, ok: true },
      { tipooperacionid: 2, ok: false, mensaje: 'Tiene registros relacionados.' },
    ];
  ...
  it('el borrado en lote pide confirmación', async () => {
    ...
    c.onSeleccion([{ tipooperacionid: 1 }] as never);
    c.onEliminarSeleccionadas();
    await asentar(fixture);
  ...
  it('pagina en cliente: el endpoint devuelve el catálogo entero', async () => {
    ...
    api.rows = Array.from({ length: 30 }, (_, i) =>
      fila({ tipooperacionid: i + 1, nombre: `FILA ${i + 1}`, orden: i + 1 }),
    );
