...
import { TipoTributoApi } from '@phoenix/basic/data-access';
import type { PageMeta } from '@phoenix/shared/api';
import {
  TIPO_TRIBUTO_MAX_ABREVIATURA,
  TIPO_TRIBUTO_MAX_CODIGO,
  TIPO_TRIBUTO_MAX_NOMBRE,
  type TipoTributo,
  type TipoTributoInput,
...
/**
 * Mantenimiento de Tipos de tributo.
 *
 ...
 * tiene.
 * El formulario lleva cinco campos porque el catálogo guarda, además del nombre, los tres
 * códigos con los que el tributo viaja al XML del comprobante.
 */
@Component({
  selector: 'erp-tipo-tributo-list-page',
  ...
  templateUrl: './tipo-tributo-list-page.html',
  styleUrl: './tipo-tributo-list-page.scss',
})
export class TipoTributoListPage {
  private readonly api = inject(TipoTributoApi);
  ...
  protected readonly maxAbreviatura = TIPO_TRIBUTO_MAX_ABREVIATURA;
  protected readonly maxCodigo = TIPO_TRIBUTO_MAX_CODIGO;
  protected readonly maxNombre = TIPO_TRIBUTO_MAX_NOMBRE;
   ...
   */
  private readonly items = linkedSignal<TipoTributo[]>(() => this.catalogo.value() ?? []);
  ...
  protected readonly error = computed(() => {
    ...
    return e instanceof Error
      ? e.message
      : e
        ? 'No se pudo cargar el catálogo de tipos de tributo.'
        : null;
  });
   ...
   */
  protected readonly rows = computed<TipoTributo[]>(() => {
    ...
    return this.items().filter((t) => {
      ...
      return t.nombre.toLocaleLowerCase('es').includes(texto) || t.abreviatura.toLocaleLowerCase('es').includes(texto) || t.codigo_tipo.toLocaleLowerCase('es').includes(texto) || t.codigo_categoria.toLocaleLowerCase('es').includes(texto) || t.codigo_contable.toLocaleLowerCase('es').includes(texto);
    });
  });
  ...
  protected readonly rowsPagina = computed<TipoTributo[]>(() => {
  ...
  // está viendo.
  protected readonly columns: GridColumn<TipoTributo>[] = [
    { key: 'orden', header: '#', width: '60px', align: 'right', mono: true },
    { key: 'nombre', header: 'Nombre' },
    { key: 'abreviatura', header: 'Abreviatura', width: '140px' },
    { key: 'codigo_tipo', header: 'Tipo', width: '90px' },
    { key: 'codigo_categoria', header: 'Categoría', width: '110px' },
    { key: 'codigo_contable', header: 'Contable', width: '110px' },
   ...
   */
  protected async onToggleEstado(row: TipoTributo): Promise<void> {
  ...
  private patchRow(id: number, patch: Partial<TipoTributo>): void {
    this.items.update((list) => list.map((it) => (it.tipoid === id ? { ...it, ...patch } : it)));
  }

  // ── Selección de filas (borrado en lote) ────────────────────────────
  protected readonly seleccionadas = signal<TipoTributo[]>([]);

  protected onSeleccion(filas: TipoTributo[]): void {
  ...
  protected async onEliminarSeleccionadas(): Promise<void> {
    ...
    const ok = await this.confirm.ask({
      title: 'Eliminar tipos de tributo',
   ...
   */
  protected async onReorder(filas: TipoTributo[]): Promise<void> {
    ...
    try {
      await firstValueFrom(this.api.reordenar(filas.map((f) => f.tipoid)));
    } catch {
  ...
  protected readonly editando = signal<TipoTributo | null>(null);
  ...
  protected readonly form = this.fb.nonNullable.group({
    nombre: ['', [Validators.required, Validators.maxLength(TIPO_TRIBUTO_MAX_NOMBRE)]],
    abreviatura: ['', [Validators.maxLength(TIPO_TRIBUTO_MAX_ABREVIATURA)]],
    codigo_tipo: ['', [Validators.maxLength(TIPO_TRIBUTO_MAX_CODIGO)]],
    codigo_categoria: ['', [Validators.maxLength(TIPO_TRIBUTO_MAX_CODIGO)]],
    codigo_contable: ['', [Validators.maxLength(TIPO_TRIBUTO_MAX_CODIGO)]],
  });

  protected readonly tituloModal = computed(() =>
    this.editando() ? 'Editar tipo de tributo' : 'Nuevo tipo de tributo',
  );

  protected onNuevo(): void {
    this.editando.set(null);
    this.form.reset({ nombre: '', abreviatura: '', codigo_tipo: '', codigo_categoria: '', codigo_contable: '' });
    this.modalAbierto.set(true);
  }

  protected onEditar(row: TipoTributo): void {
  ...
  protected async onGuardar(): Promise<void> {
    ...
    const input: TipoTributoInput = this.form.getRawValue();
  ...
  // ── Eliminación ─────────────────────────────────────────────────────
  protected async onEliminar(row: TipoTributo): Promise<void> {
    const ok = await this.confirm.ask({
      title: 'Eliminar tipo de tributo',
    ...
    try {
      ...
      // 409 si alguna venta usa el tributo: ya lo notificó el interceptor. Se recarga por si el
  ...
  protected onExportar(e: ExportChoice): void {
    const cabeceras = ['Orden', 'ID', 'Nombre', 'Abreviatura', 'Tipo', 'Categoría', 'Contable', 'Estado'];
    const matriz = [
      cabeceras,
      ...this.rows().map((t) => [
        t.orden,
    ...
    if (e.formato === 'csv') {
      exportCsv('tipos-tributo.csv', matriz);
    } else {
      exportXls('tipos-tributo.xls', matriz);
    }
  }
}
