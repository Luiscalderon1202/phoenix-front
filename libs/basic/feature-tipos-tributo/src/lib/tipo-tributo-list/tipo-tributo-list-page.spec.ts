...
import type { TipoTributo, TipoTributoInput } from '@phoenix/basic/domain';
import { ConfirmService, NotificationService } from '@phoenix/shared/ui';
import { TipoTributoListPage } from './tipo-tributo-list-page';

function fila(over: Partial<TipoTributo> = {}): TipoTributo {
  return {
    tipoid: 1,
    nombre: 'IGV',
    abreviatura: 'IGV',
...
class TipoTributoApiMock {
  rows: TipoTributo[] = [fila(), fila({ tipoid: 2, nombre: 'ICBPER', abreviatura: 'BOL', codigo_tipo: '7152', codigo_categoria: 'S', codigo_contable: '40116', orden: 2 })];

  readonly alternados: number[] = [];
  readonly eliminados: number[] = [];
  readonly creados: TipoTributoInput[] = [];
  readonly actualizados: { id: number; input: TipoTributoInput }[] = [];
  ...
  update(id: number, input: TipoTributoInput) {
...
async function setup(api = new TipoTributoApiMock()) {
  const confirm = new ConfirmServiceMock();
  const notify = new NotificationServiceMock();

  TestBed.configureTestingModule({
    imports: [TipoTributoListPage],
    providers: [
      // `erp-page-header` monta los breadcrumbs, que leen la ruta activa.
      provideRouter([]),
      { provide: TipoTributoApi, useValue: api },
      { provide: ConfirmService, useValue: confirm },
      { provide: NotificationService, useValue: notify },
    ],
  });

  const fixture = TestBed.createComponent(TipoTributoListPage);
...
function comp(fixture: { componentInstance: unknown }) {
  return fixture.componentInstance as unknown as Record<string, (...a: never[]) => unknown> & {
    ...
    form: { patchValue(v: unknown): void; getRawValue(): TipoTributoInput };
    rows(): TipoTributo[];
    filtrando(): boolean;
...
describe('TipoTributoListPage', () => {
  ...
  it('pinta el catálogo que devuelve el backend', async () => {
    ...
    expect(celdas(el, 1)).toEqual(['IGV', 'ICBPER']);
  });
  ...
  it('filtra por estado', async () => {
    const api = new TipoTributoApiMock();
  ...
  it('al reordenar manda la lista completa y SIN offset, y renumera en local', async () => {
    ...
    // desde 1, asi que mandar un tramo suelto lo colaria delante del resto.
  ...
  it('el alta manda solo los campos del formulario', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'EXONERADO', abreviatura: 'EXO', codigo_tipo: '9997', codigo_categoria: 'E', codigo_contable: '40119' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.creados).toEqual([{ nombre: 'EXONERADO', abreviatura: 'EXO', codigo_tipo: '9997', codigo_categoria: 'E', codigo_contable: '40119' }]);
  ...
  it('el formulario no ofrece ni orden ni estado', async () => {
    ...
    expect(c.form.getRawValue()).toEqual({ nombre: '', abreviatura: '', codigo_tipo: '', codigo_categoria: '', codigo_contable: '' });
  ...
  it('el alta añade la fila al final, que es donde la coloca el backend', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'EXONERADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.rows().map((row) => row.nombre).at(-1)).toBe('EXONERADO');
  });
  ...
  it('deja el modal abierto con lo escrito si el guardado falla', async () => {
    ...
    c.onNuevo();
    c.form.patchValue({ nombre: 'IGV' });
    c.onGuardar();
    await asentar(fixture);

    expect(c.modalAbierto()).toBe(true);
    expect(c.form.getRawValue().nombre).toBe('IGV');
  });
  ...
  it('editar precarga los campos editables y manda un PUT', async () => {
    ...
    expect(c.tituloModal()).toBe('Editar tipo de tributo');
    expect(c.form.getRawValue()).toEqual({ nombre: 'IGV', abreviatura: 'IGV', codigo_tipo: '1000', codigo_categoria: 'S', codigo_contable: '40111' });

    c.form.patchValue({ nombre: 'IGV EDITADO' });
    c.onGuardar();
    await asentar(fixture);

    expect(api.actualizados.length).toBe(1);
    expect(api.actualizados[0].id).toBe(1);
    expect(api.actualizados[0].input.nombre).toBe('IGV EDITADO');
    expect(api.creados).toEqual([]);
  });
  ...
  it('la edición no cambia el estado del registro', async () => {
    ...
    const api = new TipoTributoApiMock();
    api.rows = [fila(), fila({ tipoid: 2, nombre: 'ICBPER', estado: false })];
    const { fixture } = await setup(api);
    const c = comp(fixture);

    c.onEditar(api.rows[1] as never);
    c.form.patchValue({ nombre: 'ICBPER EDITADO' });
    c.onGuardar();
    await asentar(fixture);
